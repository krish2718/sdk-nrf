...
ZTEST(wifi_crypto, test_psa_import_key_failure_exact_params)
{
	 ...
	 * suite: 0x000fac02
	 * type: 32770
	 * Importing key to PSA, location: 8408663, persistence: 1, lifetime: -2142349567 key: 0x3F414201
	 * Expect failure: wifi_install_key_to_crypto: Failed to import key to PSA: -135
	 */

	uint32_t ccmp256_key[8] = {
		0x0C0D0E0F, 0x08090A0B, 0x04050607, 0x00010203,
		0xF2BDD52F, 0x514A8A19, 0xCE371185, 0xC97C1F67
	};

	const wifi_crypto_key_type_t type = PEER_UCST_ENC;
	const uint32_t db_id = 1;
	const uint32_t key_idx = 1;

	psa_key_attributes_t attr = wifi_crypto_key_attributes_init(type, db_id, key_idx);
	uint32_t key_length = wifi_crypto_get_key_size_in_bytes(type);
	psa_key_id_t key_id;
	psa_status_t status;

	// The attr struct does not have a .core member (per build error/log), so set the attributes using PSA setter APIs
	...
	psa_set_key_lifetime(&attr, (psa_key_lifetime_t)(0x80000001)); // persistence: 1, location: 0x80000000
	...
	LOG_INF("alg: %d type: %u len: %u",
		psa_get_key_algorithm(&attr),
		psa_get_key_type(&attr),
		key_length);
	...
	/* Exactly as in the log trace, expect -135 (PSA_ERROR_INVALID_HANDLE or similar) or failure. */
