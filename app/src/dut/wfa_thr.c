...
void wfaSetThreadPrio(int tid, int userPriority)
{
	...
	if (tid >= WFA_THREADS_NUM)
	{
		DPRINT_ERR(WFA_ERR, "wfaSetThreadPrio: invalid tid %d \n", tid);
		return;
	}

	tattr = &wmm_thr[tid].attr;

	/* posix priorities, Zephyr priority is based on below formula (reverse engg)
	 * zephyr_prio = CONFIG_NUM_PREEMPT_PRIORITIES - posix_prio
	 */
	switch(userPriority)
	{
		case TG_WMM_AC_BK:
			tschedParam.sched_priority = CONFIG_WFA_WMM_THREAD_PRIO_BK;
			break;
		case TG_WMM_AC_VI:
			tschedParam.sched_priority = CONFIG_WFA_WMM_THREAD_PRIO_VI;
			break;
		case TG_WMM_AC_VO:
			tschedParam.sched_priority = CONFIG_WFA_WMM_THREAD_PRIO_VO;
			break;
		case TG_WMM_AC_BE:
			tschedParam.sched_priority = CONFIG_WFA_WMM_THREAD_PRIO_BE;
			break;
		case TG_WMM_AC_RX:
			tschedParam.sched_priority = CONFIG_WFA_WMM_THREAD_PRIO_RX;
			break;
		default:
			/* default */
			;
	...
	/* This takes POSIX priority and converts to Zephyr automatically.
	 * SCHED_RR is pre-emptive threads and SCHED_FIFO is co-operative threads
	 */
	pthread_setschedparam(wmm_thr[tid].thr, SCHED_RR, &tschedParam);
}
...
void * wfa_wmm_thread(void *thr_param)
{
	...
	while(1)
	{
		...
		if (myStream == NULL)
		{
			DPRINT_ERR(WFA_ERR, "wfa_wmm_thread::findStreamProfile error for stream %d\n", myStreamId);
		...
		switch(myProfile->direction)
		{
			...
			case DIRECT_RECV:
				...
				else if (myProfile->profile == PROF_IPTV || myProfile->profile == PROF_FILE_TX || myProfile->profile == PROF_MCAST)
				{
					...
					wfaSetThreadPrio(myId, TG_WMM_AC_RX);
					char *recvBuf;
