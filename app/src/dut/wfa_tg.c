...
int wfaTGConfig(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
	...
	myStream->id = ++streamId; /* the id start from 1 */
	myStream->tblidx = slotCnt-1;
	DPRINT_INFO(WFA_OUT, "entering tcConfig ...%d\n", myStream->id);
	...
	while ( runLoop)
	{
		...
		for ( i=0; i<= (theProf->rate); i++)
		{
			...
			else
			{
				...
				DPRINT_INFO(WFA_OUT, "wfaSendBitrateData wfaTrafficSendTo call ERR counter=%i i=%i; streamId=%d send busy, sleep %i MilSec then send\n", counter, i, streamId, sleepTimePerSent);
				i--;
			}
