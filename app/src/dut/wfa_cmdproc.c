...
int xcCmdProcStaSetSecurity(char *pcmdStr, BYTE *aBuf, int *aLen)
{
	...
	for(;;)
	{
		...
		if(str == NULL || str[0] == '\0')
			break;
		DPRINT_INFO(WFA_OUT, "IMG DEBUG STRING FOR COMP %s\n", str);
		...
		else if(strcasecmp(str, "keyMgmtType") == 0)
		{
			...
			if(strcasecmp(str, "SuiteB") == 0 || strcasecmp(str, "SAE") == 0|| strcasecmp(str, "OWE") == 0)
		...
		else if(strcasecmp(str, "encpType") == 0)
		{
			...
			if(strcasecmp(str, "tkip") == 0 || strcasecmp(str, "aes-ccmp") == 0 || strcasecmp(str, "aes-gcmp") == 0)
				strncpy(ssec->encpType, str, 9);
		}
		else if(strcasecmp(str, "certType") == 0)
		{
			str = strtok_r(NULL, ",", &pcmdStr);
			strcpy(ssec->certType, str);
		}
		else if(strcasecmp(str, "trustedRootCA") == 0)
		{
			str = strtok_r(NULL, ",", &pcmdStr);
			strcpy(ssec->trustedRootCA, str);
		}
		else if(strcasecmp(str, "clientCertificate") == 0)
		{
			str = strtok_r(NULL, ",", &pcmdStr);
			strcpy(ssec->clientCertificate, str);
		}        
		...
		else if(strcasecmp(str, "type") == 0)
		{
			...
			str = strtok_r (NULL, ",", &pcmdStr);
			DPRINT_INFO(WFA_OUT, "IMG DEBUG STRING FOR secType %s\n", str);
			...
			{
				ssec->type = secType = SEC_TYPE_PSKSAE;
				printf("\n DDEBUG: %s inside psk-sae\n", __func__);
			}
...
int xcCmdProcStaDevSendFrame(char *pcmdStr, BYTE *aBuf, int *aLen)
{
	...
	for(;;)
	{
		...
		{
			str = strtok_r(NULL, ",", &pcmdStr);
			sf->BTMQuery_Reason_Code = atoi(str);
		}
		else if(strcasecmp(str, "WNM_Notify_Element") == 0)
		{
			str = strtok_r(NULL, ",", &pcmdStr);
			strncpy(sf->WNM_Notify_Element, str, 16);
		}
			...
			else if (strcasecmp(str, "MBO") == 0)
			{
				...
				for(;;)
				{
					...
					if (strcasecmp(str, "framename") == 0)
					{
						...
						if (strcasecmp(str, "ANQPQuery") == 0)
						{
							mbo->eframe= MBO_FRAME_ANQPQuery;
						}
						if (strcasecmp(str, "BTMQuery") == 0)
						{
							mbo->eframe= MBO_FRAME_BTMQuery;
						}
						if (strcasecmp(str, "BTMReq") == 0)
						{
							mbo->eframe= MBO_FRAME_BTMReq;
						}
						if (strcasecmp(str, "BcnRptReq") == 0)
						{
							mbo->eframe= MBO_FRAME_BcnRptReq;
						}
						if (strcasecmp(str, "WNM_Notify") == 0)
						{
							mbo->eframe= MBO_FRAME_WNM_Notify;
						}
						if (strcasecmp(str, "disassoc") == 0)
						{
					...
					else if(strcasecmp(str, "Name") == 0)
					{
						...
						strncpy(mbo->sDevName, str, WFA_SSID_NAME_LEN-1);
						mbo->sDevName[WFA_SSID_NAME_LEN-1]='\0';
					}
					else if(strcasecmp(str, "Dest_MAC") == 0)
					{
						str = strtok_r(NULL, ",", &pcmdStr);
						strncpy(mbo->sDestMac, str, WFA_MAC_ADDR_STR_LEN-1);
						mbo->sDestMac[WFA_MAC_ADDR_STR_LEN-1]='\0';
					}
					else if(strcasecmp(str, "Cand_List") == 0)
					{
						str = strtok_r(NULL, ",", &pcmdStr);
						mbo->CandList = atoi(str);
					}
				}
			}/*End of MBO*/
		}/* program  */
...
int xcCmdProcStaPresetTestParameters(char *pcmdStr, BYTE *aBuf, int *aLen)
{
	...
	for(;;)
	{
		...
		else if (strcasecmp(str, "Ch_Pref_Num")==0)
		{
			...
			DPRINT_INFO(WFA_OUT, "Ch_Pref_Num %d\n", presetTestParams->Ch_Pref_Num);
		...
		else if (strcasecmp(str, "Ch_Op_Class")==0)
		{
			...
			DPRINT_INFO(WFA_OUT, "Ch_Op_Class  %d\n", presetTestParams->Ch_Op_Class);
...
int xcCmdProcStaSetRFeature(char *pcmdStr, BYTE *aBuf, int *aLen)
{
...
#endif
	char *str;
	printf("In func xcCmdProcStaSetRFeature\n");
	DPRINT_INFO(WFA_OUT, "Entering xcCmdProcStaSetRFeature...\n");
	if(aBuf == NULL)
		return WFA_FAILURE;

	memset(aBuf, 0, *aLen);
	memset(rfeat, 0, sizeof(caStaRFeat_t));
#ifdef CONFIG_USB_DEVICE_STACK
    for(;;)
    {
        str = strtok_r(NULL, ",", &pcmdStr);
   ...
   printf("pcmdStr : %s",str);
        ...
        {
            str = strtok_r(NULL, ",", &pcmdStr);
            strncpy(rfeat->twt_setup, str, sizeof(rfeat->twt_setup));
        }
         else if(strcasecmp(str, "NDPPagingInd") == 0)
        {
            str = strtok_r(NULL, ",", &pcmdStr);
            rfeat->twt_ndppagingind = atoi(str);
        }
         else if(strcasecmp(str, "RespPMMode") == 0)
        {
            str = strtok_r(NULL, ",", &pcmdStr);
            rfeat->twt_resppmmode = atoi(str);
        }
         else if(strcasecmp(str, "NegotiationType") == 0)
        {
            str = strtok_r(NULL, ",", &pcmdStr);
            rfeat->twt_negotiationtype = atoi(str);
        }
         else if(strcasecmp(str, "SetupCommand") == 0)
        {
            str = strtok_r(NULL, ",", &pcmdStr);
            rfeat->twt_setup_command = atoi(str);
        }
...
int xcCmdProcStaScan(char *pcmdStr, BYTE *aBuf, int *aLen)
{
	...
	wfaEncodeTLV(WFA_STA_SCAN_TLV, sizeof(caStaScan_t), (BYTE *)staScan, aBuf);
