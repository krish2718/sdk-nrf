...
static int setup_twt(void)
{
	...
	params.setup.twt_interval = 4 * CONFIG_TWT_INTERVAL;
...
static void handle_wifi_twt_event(struct net_mgmt_event_callback *cb)
{
	...
	if (resp->operation == WIFI_TWT_TEARDOWN) {
		// LOG_INF("twt event handled!!!!!!!!!!!!!!!!!!!");

		// LOG_INF("resp->dialog_token: %d\n", resp->dialog_token);
		// LOG_INF("resp->fail_reason: %d\n", resp->fail_reason);
		// LOG_INF("resp->flow_id: %d\n", resp->flow_id);
		// LOG_INF("resp->negotiation_type: %d\n", resp->negotiation_type);
		// LOG_INF("resp->operation: %d\n", resp->operation);
		// LOG_INF("resp->resp_status: %d\n", resp->resp_status);
		// LOG_INF("resp->setup.announce: %d\n", resp->setup.announce);
		// LOG_INF("resp->setup.implicit: %d\n", resp->setup.implicit);
		// LOG_INF("resp->setup.responder: %d\n", resp->setup.responder);
		// LOG_INF("resp->setup.trigger: %d\n", resp->setup.trigger);
		// LOG_INF("resp->setup.twt_interval: %d\n", resp->setup.twt_interval);
		// LOG_INF("resp->setup.twt_wake_ahead_duration: %d\n", resp->setup.twt_wake_ahead_duration);
		// LOG_INF("resp->setup.twt_wake_interval: %d\n", resp->setup.twt_wake_interval);
		// LOG_INF("resp->setup_cmd: %d\n", resp->setup_cmd);
		// LOG_INF("resp->teardown.teardown_all: %d\n", resp->teardown.teardown_all);
		// LOG_INF("resp->teardown_status: %d\n", resp->teardown_status);
...
int main(void)
{
	...
	while (1) {
		...
		if (context.connected) {
			...
			k_sleep(K_USEC(20 * CONFIG_TWT_INTERVAL));
