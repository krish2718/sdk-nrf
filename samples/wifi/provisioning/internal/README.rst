...
The sample uses the core Wi-Fi provisioning library (``wifi_prov_core``) with a
transport stub implementation that decodes and logs protobuf messages.
This allows testing of the provisioning protocol without requiring a Bluetooth
transport layer.
...
- *Transport Decoupling*: Uses the core Wi-Fi provisioning library without
...
.. list-table:: Shell commands for Wi-Fi provisioning
   ...
   * - ``wifi_prov raw <base64_data>``
     ...
     - ``wifi_prov raw CARaLgoaCgpTYW1wbGVXaUZpEgYAESIzRFUYAiAAKAMSDnNhbXBsZXBhc3N3b3JkIAA=``
   * - ``wifi_prov dump_scan <base64_data>``
     ...
     - ``wifi_prov dump_scan <base64_encoded_scan_results>``
