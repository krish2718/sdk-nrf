...
LOG_MODULE_REGISTER(prov, CONFIG_BT_WIFI_PROV_LOG_LEVEL);

/* Generated WiFi configuration data */
...
/**
 * @brief Convert base64 string to binary data
 */
static int base64_decode(const char *base64_str, uint8_t **binary, size_t *binary_len)
{
	...
	for (size_t i = 0; i < len; i++) {
		...
		if (bits >= 8) {
			decoded_data[j++] = (val >> (bits - 8)) & 0xFF;
			bits -= 8;
		}
	}
...
/**
 * @brief Decode and log WiFi provisioning request
 */
static void decode_request(const uint8_t *data, size_t len)
{
	...
	if (decode_result) {
		LOG_INF("=== WiFi Provisioning Request ===");
		...
		case OpCode_SET_CONFIG:
			...
			if (request.has_config) {
				...
				LOG_INF("  BSSID: %02x:%02x:%02x:%02x:%02x:%02x",
					   request.config.wifi.bssid.bytes[0], request.config.wifi.bssid.bytes[1],
					   request.config.wifi.bssid.bytes[2], request.config.wifi.bssid.bytes[3],
					   request.config.wifi.bssid.bytes[4], request.config.wifi.bssid.bytes[5]);
				...
				if (request.config.has_passphrase) {
					LOG_INF("  Passphrase length: %d", (int)request.config.passphrase.size);
				}
			}
			break;
...
static void decode_scan_results(const uint8_t *data, size_t len)
{
	...
	if (decode_result) {
		...
		if (scan_record.has_wifi) {
			...
			/* SSID */
			if (wifi->ssid.size > 0) {
				LOG_INF("SSID: %.*s", (int)wifi->ssid.size, wifi->ssid.bytes);
			} else {
				LOG_INF("SSID: <hidden>");
			}
			...
			if (wifi->bssid.size == 6) {
				LOG_INF("BSSID: %02x:%02x:%02x:%02x:%02x:%02x",
					wifi->bssid.bytes[0], wifi->bssid.bytes[1],
					wifi->bssid.bytes[2], wifi->bssid.bytes[3],
					wifi->bssid.bytes[4], wifi->bssid.bytes[5]);
			}
...
/**
 * @brief WiFi provisioning shell command handler
 *
 * This command sends the pre-generated WiFi configuration data
 * to the WiFi provisioning service.
 */
static int cmd_wifi_prov(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_info(shell, "Sending WiFi configuration to provisioning service...");
	...
	/* Send the configuration to the WiFi provisioning service */
	ret = wifi_prov_recv_req(req_buf);
	if (ret < 0) {
		shell_error(shell, "Failed to send WiFi configuration: %d", ret);
		return ret;
	}

	shell_info(shell, "WiFi configuration sent successfully");
...
/**
 * @brief Send raw binary data to WiFi provisioning service
 */
static int cmd_wifi_prov_raw(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (argc != 2) {
		...
		shell_error(shell, "Example: wifi_prov_raw CARaLgoaCgpTYW1wbGVXaUZpEgYAESIzRFUYAiAAKAMSDnNhbXBsZXBhc3N3b3JkIAA=");
		return -EINVAL;
	}
	...
	ret = base64_decode(argv[1], &raw_data, &raw_data_len);
	if (ret < 0) {
	...
	/* Send the raw data to the WiFi provisioning service */
...
/**
 * @brief Get WiFi provisioning status
 */
static int cmd_wifi_prov_get_status(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_info(shell, "Getting WiFi provisioning status...");
	...
	if (ret < 0) {
		shell_error(shell, "Failed to get WiFi status: %d", ret);
		return ret;
	}

	shell_info(shell, "WiFi status request sent successfully");
	return 0;
}
...
static int cmd_wifi_prov_start_scan(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_info(shell, "Starting WiFi scan...");
	...
	if (ret < 0) {
		shell_error(shell, "Failed to start WiFi scan: %d", ret);
		return ret;
	}

	shell_info(shell, "WiFi scan started successfully");
	return 0;
}
...
static int cmd_wifi_prov_stop_scan(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_info(shell, "Stopping WiFi scan...");
	...
	net_buf_simple_init_with_data(&stop_scan_req_buf, stop_scan_req_data, sizeof(stop_scan_req_data));
	...
	if (ret < 0) {
		shell_error(shell, "Failed to stop WiFi scan: %d", ret);
		return ret;
	}

	shell_info(shell, "WiFi scan stopped successfully");
	return 0;
}
...
static int cmd_wifi_prov_set_config(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_info(shell, "Setting WiFi configuration...");

	/* Use the pre-generated WiFi configuration */
	...
	if (ret < 0) {
		shell_error(shell, "Failed to set WiFi configuration: %d", ret);
		return ret;
	}

	shell_info(shell, "WiFi configuration set successfully");
	return 0;
}

/**
 * @brief Forget WiFi configuration
 */
static int cmd_wifi_prov_forget_config(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_info(shell, "Forgetting WiFi configuration...");
	...
	if (ret < 0) {
		shell_error(shell, "Failed to forget WiFi configuration: %d", ret);
		return ret;
	}

	shell_info(shell, "WiFi configuration forgotten successfully");
	return 0;
}
...
static int cmd_wifi_prov_dump_scan(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (argc != 2) {
		...
		shell_error(shell, "Example: wifi_prov dump_scan <base64_encoded_scan_results_data>");
		return -EINVAL;
	}
	...
	ret = base64_decode(argv[1], &scan_data, &scan_data_len);
	if (ret < 0) {
...
/**
 * @brief WiFi provisioning status shell command handler
 *
 * This command shows information about the generated WiFi configuration.
 */
static int cmd_wifi_prov_info(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_info(shell, "WiFi Configuration Information:");
	...
	shell_info(shell, "=== Decoded WiFi Configuration ===");
...
SHELL_STATIC_SUBCMD_SET_CREATE(wifi_prov_subcmds,
	SHELL_CMD(info, NULL, "Show WiFi configuration information", cmd_wifi_prov_info),
	SHELL_CMD(get_status, NULL, "Get WiFi provisioning status", cmd_wifi_prov_get_status),
	SHELL_CMD(start_scan, NULL, "Start WiFi scan", cmd_wifi_prov_start_scan),
	SHELL_CMD(stop_scan, NULL, "Stop WiFi scan", cmd_wifi_prov_stop_scan),
	SHELL_CMD(set_config, NULL, "Set WiFi configuration", cmd_wifi_prov_set_config),
	SHELL_CMD(forget_config, NULL, "Forget WiFi configuration", cmd_wifi_prov_forget_config),
	SHELL_CMD(raw, NULL, "Send raw binary data (hex format)", cmd_wifi_prov_raw),
	SHELL_CMD(dump_scan, NULL, "Decode and display scan results (Base64)", cmd_wifi_prov_dump_scan),
	SHELL_SUBCMD_SET_END
);

SHELL_CMD_REGISTER(wifi_prov, &wifi_prov_subcmds, "WiFi provisioning commands", cmd_wifi_prov);
