...
LOG_MODULE_REGISTER(wifi_prov_transport, CONFIG_BT_WIFI_PROV_LOG_LEVEL);

/**
 * @brief Decode and log WiFi provisioning response
 */
static void decode_response(const uint8_t *data, size_t len)
{
	...
	bool decode_result = pb_decode(&stream, Response_fields, &response);

	if (decode_result) {
		LOG_INF("=== WiFi Provisioning Response ===");
		...
		if (response.has_device_status) {
			...
			if (response.device_status.has_provisioning_info) {
				...
				LOG_INF("    SSID: %.*s", (int)response.device_status.provisioning_info.ssid.size,
				...
				LOG_INF("    Channel: %d", response.device_status.provisioning_info.channel);
				LOG_INF("    Auth: %d", response.device_status.provisioning_info.auth);
			}
			...
			if (response.device_status.has_scan_info) {
				...
				LOG_INF("    Passive: %s", response.device_status.scan_info.passive ? "true" : "false");
				LOG_INF("    Period (ms): %d", response.device_status.scan_info.period_ms);
				LOG_INF("    Group Channels: %d", response.device_status.scan_info.group_channels);
			}
		}
...
/**
 * @brief Decode and log WiFi provisioning result
 */
static void decode_result(const uint8_t *data, size_t len)
{
	...
	bool decode_result = pb_decode(&stream, Result_fields, &result);

	if (decode_result) {
		LOG_INF("=== WiFi Provisioning Result ===");
		...
		if (result.has_scan_record) {
			...
			if (result.scan_record.has_wifi) {
				LOG_INF("  SSID: %.*s", (int)result.scan_record.wifi.ssid.size,
...
{
	LOG_INF("WiFi provisioning response sent (stub): %d bytes", rsp->len);
...
{
	LOG_INF("WiFi provisioning result sent (stub): %d bytes", result->len);
