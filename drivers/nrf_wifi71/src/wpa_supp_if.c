...
int nrf_wifi_wpa_supp_scan2(void *if_priv, struct wpa_driver_scan_params *params)
{
	...
	scan_info =
		nrf_wifi_osal_mem_zalloc(sizeof(*scan_info) + (num_freqs * sizeof(unsigned int)));
	...
	k_work_schedule(&vif_ctx_zep->scan_timeout_work,
			K_SECONDS(CONFIG_WIFI_NRF71_SCAN_TIMEOUT_S));
...
int nrf_wifi_wpa_set_supp_port(void *if_priv, int authorized, char *bssid)
{
	...
	status = nrf_wifi_sys_fmac_chg_sta(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
					   &chg_sta_info);
...
int nrf_wifi_wpa_supp_signal_poll(void *if_priv, struct wpa_signal_info *si, unsigned char *bssid)
{
	...
	enum nrf71_wifi_status ret = NRF71_WIFI_STATUS_FAIL;
	int sem_ret;
	...
	if (rssi_record_elapsed_time_ms > CONFIG_NRF71_RSSI_STALE_TIMEOUT_MS) {
		ret = nrf_wifi_sys_fmac_get_station(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
						    bssid);
			       ...
			       int cookie)
{
	...
	LOG_DBG("%s: Sending frame to RPU: cookie=%d wait_time=%d no_ack=%d", __func__, cookie,
		wait_time, noack);
	status =
		nrf_wifi_sys_fmac_mgmt_tx(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx, mgmt_tx_info);
...
enum nrf71_wifi_status nrf_wifi_parse_sband(struct nrf_wifi_event_supported_band *event,
					  struct wpa_supp_event_supported_band *band)
{
	int count;

	if (event && (event->nrf_wifi_n_bitrates == 0 || event->nrf_wifi_n_channels == 0)) {
		return NRF71_WIFI_STATUS_FAIL;
	}
	...
	if ((wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_EXTENDED_CAPABILITIES) &&
		...
		if (rpu_ctx_zep->extended_capa_mask) {
			memcpy(rpu_ctx_zep->extended_capa_mask,
			       wiphy_info->extended_capabilities_mask, ec_len);
		} else {
...
{
	enum nrf71_wifi_status status = NRF71_WIFI_STATUS_SUCCESS;
	struct nrf_wifi_vif_ctx_zep *vif_ctx_zep = NULL;
	struct nrf_wifi_ctx_zep *rpu_ctx_zep = NULL;
...
static int nrf_wifi_iftype_change(struct nrf_wifi_vif_ctx_zep *vif_ctx_zep, int iftype)
{
	...
	status = nrf_wifi_sys_fmac_chg_vif(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
					   &chg_vif_info);
	if (status != NRF71_WIFI_STATUS_SUCCESS) {
	...
	if (vif_ctx_zep->set_if_status != NRF71_WIFI_STATUS_SUCCESS) {
				      ...
				      const u8 *match)
{
	...
	status = nrf_wifi_sys_fmac_mgmt_frame_reg(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
						  &mgmt_frame_info);
	if (status != NRF71_WIFI_STATUS_SUCCESS) {
...
int nrf_wifi_wpa_supp_start_ap(void *if_priv, struct wpa_driver_ap_params *params)
{
	...
	status = nrf_wifi_sys_fmac_start_ap(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
					    &start_ap_info);
	if (status != NRF71_WIFI_STATUS_SUCCESS) {
...
int nrf_wifi_wpa_supp_change_beacon(void *if_priv, struct wpa_driver_ap_params *params)
{
	...
	status = nrf_wifi_sys_fmac_chg_bcn(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
					   &chg_bcn_info);
	if (status != NRF71_WIFI_STATUS_SUCCESS) {
...
int nrf_wifi_wpa_supp_sta_add(void *if_priv, struct hostapd_sta_add_params *params)
{
	...
	if (params->set) {
		...
		status = nrf_wifi_sys_fmac_add_sta(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
						   &sta_info);
	}
	if (status != NRF71_WIFI_STATUS_SUCCESS) {
...
int nrf_wifi_wpa_supp_sta_get_inact_sec(void *if_priv, const u8 *addr)
{
	...
	status = nrf_wifi_sys_fmac_get_station(rpu_ctx_zep->rpu_ctx, vif_ctx_zep->vif_idx,
					       (unsigned char *)addr);
	if (status != NRF71_WIFI_STATUS_SUCCESS) {
