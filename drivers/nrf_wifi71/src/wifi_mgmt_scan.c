			   ...
			   scan_result_cb_t cb)
{
	...
	scan_info = nrf_wifi_osal_mem_zalloc(
		sizeof(*scan_info) +
	...
	static uint8_t skip_local_admin_mac = IS_ENABLED(CONFIG_WIFI_NRF71_SKIP_LOCAL_ADMIN_MAC);
...
enum nrf71_wifi_status nrf_wifi_disp_scan_res_get_zep(struct nrf_wifi_vif_ctx_zep *vif_ctx_zep)
{
	enum nrf71_wifi_status status = NRF71_WIFI_STATUS_FAIL;
	struct nrf_wifi_ctx_zep *rpu_ctx_zep = NULL;
