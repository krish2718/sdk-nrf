...
{
	opriv->ops->llist_node_data_set(node, data);
}
...
void wifi_nrf_osal_llist_add_node_tail(struct wifi_nrf_osal_priv *opriv, void *llist,
				       void *llist_node)
{
	return opriv->ops->llist_add_node_tail(llist, llist_node);
}
...
void *wifi_nrf_osal_llist_get_node_nxt(struct wifi_nrf_osal_priv *opriv, void *llist,
				       void *llist_node)
{
	return opriv->ops->llist_get_node_nxt(llist, llist_node);
}

void wifi_nrf_osal_llist_del_node(struct wifi_nrf_osal_priv *opriv, void *llist, void *llist_node)
{
	opriv->ops->llist_del_node(llist, llist_node);
}
...
void wifi_nrf_osal_nbuf_headroom_res(struct wifi_nrf_osal_priv *opriv, void *nbuf,
				     unsigned int size)
{
	opriv->ops->nbuf_headroom_res(nbuf, size);
}
...
{
	opriv->ops->tasklet_init(tasklet, callbk_fn, data);
}
...
{
	return opriv->ops->bus_pcie_init(dev_name, vendor_id, sub_vendor_id, device_id,
					 sub_device_id);
}
...
{
	return opriv->ops->bus_pcie_dev_add(os_pcie_priv, osal_pcie_dev_ctx);
}
...
{
	return opriv->ops->bus_pcie_dev_intr_reg(os_pcie_dev_ctx, callbk_data, callbk_fn);
}
...
{
	return opriv->ops->bus_pcie_dev_dma_map(os_pcie_dev_ctx, virt_addr, size, dir);
}
...
void wifi_nrf_osal_bus_pcie_dev_dma_unmap(struct wifi_nrf_osal_priv *opriv, void *os_pcie_dev_ctx,
					  ...
					  enum wifi_nrf_osal_dma_dir dir)
{
	opriv->ops->bus_pcie_dev_dma_unmap(os_pcie_dev_ctx, dma_addr, size, dir);
}
...
{
	opriv->ops->bus_pcie_dev_host_map_get(os_pcie_dev_ctx, host_map);
}
...
{
	return opriv->ops->bus_qspi_dev_add(os_qspi_priv, osal_qspi_dev_ctx);
}
...
enum wifi_nrf_status wifi_nrf_osal_bus_qspi_dev_intr_reg(struct wifi_nrf_osal_priv *opriv,
							 ...
							 int (*callbk_fn)(void *callbk_data))
{
	return opriv->ops->bus_qspi_dev_intr_reg(os_qspi_dev_ctx, callbk_data, callbk_fn);
}
