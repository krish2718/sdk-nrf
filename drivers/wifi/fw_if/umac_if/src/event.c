...
static void umac_if_event(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx, void *event_data)
{
	...
	event = (struct img_umac_event_vif_state *)event_data;

	if (event->umac_hdr.cmd_evnt != IMG_UMAC_EVENT_IFFLAGS_STATUS) {
		return;
	}
...
static void umac_event_connect(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx, void *event_data)
{
	...
	if (event->umac_hdr.cmd_evnt == IMG_UMAC_EVENT_NEW_STATION) {
		if (vif_ctx->if_type == 2) {
			wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, vif_ctx->bssid,
					      event->mac_addr, IMG_ETH_ALEN);
		}

		peer_id = wifi_nrf_fmac_peer_get_id(fmac_dev_ctx, event->mac_addr);

		if (peer_id == -1) {
			peer_id = wifi_nrf_fmac_peer_add(fmac_dev_ctx, if_index, event->mac_addr,
							 event->is_sta_legacy, event->wme);
		...
		peer_id = wifi_nrf_fmac_peer_get_id(fmac_dev_ctx, event->mac_addr);
		if (peer_id != -1) {
			wifi_nrf_fmac_peer_remove(fmac_dev_ctx, if_index, peer_id);
		}
	}
}

static enum wifi_nrf_status umac_event_ctrl_process(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx,
						    void *event_data, unsigned int event_len)
{
	...
	case IMG_UMAC_EVENT_SCAN_DONE:
		if (callbk_fns->scan_done_callbk_fn) {
			callbk_fns->scan_done_callbk_fn(vif_ctx->os_vif_ctx, event_data);
		} else {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      "%s: No callback registered for event %d\n", __func__,
					      umac_hdr->cmd_evnt);
	...
	case IMG_UMAC_EVENT_SCAN_RESULT:
		...
		if (callbk_fns->disp_scan_res_callbk_fn) {
			callbk_fns->scan_res_callbk_fn(vif_ctx->os_vif_ctx, event_data, more_res);
		} else {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      "%s: No callback registered for event %d\n", __func__,
					      umac_hdr->cmd_evnt);
	...
	case IMG_UMAC_EVENT_SCAN_DISPLAY_RESULT:
		...
		if (callbk_fns->disp_scan_res_callbk_fn) {
			callbk_fns->disp_scan_res_callbk_fn(vif_ctx->os_vif_ctx, event_data,
							    more_res);
		} else {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      "%s: No callback registered for event %d\n", __func__,
	...
	case IMG_UMAC_EVENT_AUTHENTICATE:
		if (callbk_fns->auth_resp_callbk_fn) {
			callbk_fns->auth_resp_callbk_fn(vif_ctx->os_vif_ctx, event_data);
		} else {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      "%s: No callback registered for event %d\n", __func__,
					      umac_hdr->cmd_evnt);
	...
	case IMG_UMAC_EVENT_ASSOCIATE:
		if (callbk_fns->assoc_resp_callbk_fn) {
			callbk_fns->assoc_resp_callbk_fn(vif_ctx->os_vif_ctx, event_data);
		} else {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      "%s: No callback registered for event %d\n", __func__,
					      umac_hdr->cmd_evnt);
	...
	case IMG_UMAC_EVENT_DEAUTHENTICATE:
		if (callbk_fns->deauth_callbk_fn) {
			callbk_fns->deauth_callbk_fn(vif_ctx->os_vif_ctx, event_data);
		} else {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      "%s: No callback registered for event %d\n", __func__,
					      umac_hdr->cmd_evnt);
	...
	case IMG_UMAC_EVENT_DISASSOCIATE:
		if (callbk_fns->disassoc_callbk_fn) {
			callbk_fns->disassoc_callbk_fn(vif_ctx->os_vif_ctx, event_data);
		} else {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv,
					      "%s: No callback registered for event %d\n", __func__,
					      umac_hdr->cmd_evnt);
...
static enum wifi_nrf_status
wifi_nrf_fmac_data_event_process(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx, void *umac_head)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	int event = -1;
	...
	case IMG_CMD_RX_BUFF:
		status = wifi_nrf_fmac_rx_event_process(fmac_dev_ctx, umac_head);
		break;
	case IMG_CMD_TX_BUFF_DONE:
		status = wifi_nrf_fmac_tx_done_event_process(fmac_dev_ctx, umac_head);
		break;
	case IMG_CMD_CARRIER_ON:
		status = wifi_nrf_fmac_if_state_chg_event_process(fmac_dev_ctx, umac_head,
	...
	case IMG_CMD_PM_MODE:
		status = sap_client_update_pmmode(fmac_dev_ctx, umac_head);
		break;
	case IMG_CMD_PS_GET_FRAMES:
		status = sap_client_ps_get_frames(fmac_dev_ctx, umac_head);
		break;
	default:
		break;
	}
...
out:
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Failed for event = %d\n",
				      __func__, event);
	}
				  ...
				  struct host_rpu_msg *rpu_msg)
{
	...
	while (host_rpu_length_left > 0) {
		status = wifi_nrf_fmac_data_event_process(fmac_dev_ctx, umac_head);
						     ...
						     void *event)
{
	...
	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, fmac_dev_ctx->fw_stats, &stats->fw,
						    ...
						    struct host_rpu_msg *rpu_msg)
{
	...
	case IMG_EVENT_STATS:
		status = umac_event_stats_process(fmac_dev_ctx, sys_head);
		break;
...
enum wifi_nrf_status wifi_nrf_fmac_event_callback(void *mac_dev_ctx, void *rpu_event_data,
						  unsigned int rpu_event_len)
{
	...
	case IMG_HOST_RPU_MSG_TYPE_DATA:
		status = wifi_nrf_fmac_data_events_process(fmac_dev_ctx, rpu_msg);
		break;
	case IMG_HOST_RPU_MSG_TYPE_UMAC:
		...
		if (umac_msg_type == IMG_UMAC_EVENT_IFFLAGS_STATUS) {
			umac_if_event(fmac_dev_ctx, rpu_msg->msg);
		}

		if ((umac_msg_type == IMG_UMAC_EVENT_NEW_STATION) ||
		    (umac_msg_type == IMG_UMAC_EVENT_DEL_STATION)) {
			umac_event_connect(fmac_dev_ctx, rpu_msg->msg);
		}

		break;
	case IMG_HOST_RPU_MSG_TYPE_SYSTEM:
		status = umac_process_sys_events(fmac_dev_ctx, rpu_msg);
		break;
	default:
		goto out;
	}
