			      ...
			      const unsigned char *mac_addr)
{
	...
	for (i = 0; i < MAX_PEERS; i++) {
		...
		if ((wifi_nrf_util_ether_addr_equal(mac_addr, (void *)peer->ra_addr))) {
			return peer->peer_id;
		}
	}
	return -1;
...
int wifi_nrf_fmac_peer_add(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx,
			   unsigned char wifi_nrf_vif_idx, const unsigned char *mac_addr,
			   unsigned char is_legacy, unsigned char qos_supported)
{
	int i;
	struct peers_info *peer;
	...
	if (wifi_nrf_util_is_multicast_addr(mac_addr) && (vif_ctx->if_type == IMG_IFTYPE_AP)) {
	...
	for (i = 0; i < MAX_PEERS; i++) {
		...
		if (peer->peer_id == -1) {
			wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, peer->ra_addr, mac_addr,
					      IMG_ETH_ALEN);
			peer->wifi_nrf_vif_idx = wifi_nrf_vif_idx;
			peer->peer_id = i;
			...
			if (vif_ctx->if_type == IMG_IFTYPE_AP) {
				hal_rpu_mem_write(fmac_dev_ctx->hal_dev_ctx,
						  ...
						  peer->ra_addr, WIFI_NRF_FMAC_ETH_ADDR_LEN);
			}
			return i;
		}
	}

	wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Failed !! No Space Available",
			      __func__);
...
void wifi_nrf_fmac_peer_remove(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx,
			       unsigned char wifi_nrf_vif_idx, int peer_id)
{
	struct peers_info *peer;
	struct wifi_nrf_fmac_vif_ctx *vif_ctx = NULL;
	...
	wifi_nrf_osal_mem_set(fmac_dev_ctx->fpriv->opriv, peer, 0x0, sizeof(struct peers_info));
	peer->peer_id = -1;
			       ...
			       unsigned char wifi_nrf_vif_idx)
{
	...
	for (i = 0; i < MAX_PEERS; i++) {
		peer = &fmac_dev_ctx->tx_config.peers[i];
		if (peer->wifi_nrf_vif_idx == wifi_nrf_vif_idx) {
			wifi_nrf_osal_mem_set(fmac_dev_ctx->fpriv->opriv, peer, 0x0,
