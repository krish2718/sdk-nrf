...
enum wifi_nrf_status hal_rpu_reg_read(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx, unsigned int *val,
				      unsigned int rpu_reg_addr)
{
	...
	if ((val == NULL) || !hal_rpu_is_reg(rpu_reg_addr)) {
	...
	status = pal_rpu_addr_offset_get(hal_dev_ctx->hpriv->opriv, rpu_reg_addr, &addr_offset);
