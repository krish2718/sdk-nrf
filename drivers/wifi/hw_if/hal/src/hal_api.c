...
wifi_nrf_hal_rpu_pktram_buf_map_init(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx)
{
	...
	status = pal_rpu_addr_offset_get(hal_dev_ctx->hpriv->opriv, RPU_MEM_PKT_BASE,
...
unsigned long wifi_nrf_hal_buf_map_rx(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx, unsigned long buf,
				      ...
				      unsigned int buf_id)
{
	...
	bounce_buf_addr = hal_dev_ctx->addr_rpu_pktram_base_rx_pool[pool_id] + (buf_id * buf_len);

	wifi_nrf_bal_write_block(hal_dev_ctx->bal_dev_ctx, (unsigned int)bounce_buf_addr,
				 (void *)buf, hal_dev_ctx->hpriv->cfg_params.rx_buf_headroom_sz);

	addr_to_map = bounce_buf_addr + hal_dev_ctx->hpriv->cfg_params.rx_buf_headroom_sz;

	rx_buf_info->phy_addr = wifi_nrf_bal_dma_map(hal_dev_ctx->bal_dev_ctx, addr_to_map, buf_len,
						     WIFI_NRF_OSAL_DMA_DIR_FROM_DEV);

	if (!rx_buf_info->phy_addr) {
		wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv, "%s: DMA map failed\n", __func__);
		goto out;
	}
...
unsigned long wifi_nrf_hal_buf_unmap_rx(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx,
					unsigned int data_len, unsigned int pool_id,
					unsigned int buf_id)
{
	struct wifi_nrf_hal_buf_map_info *rx_buf_info = NULL;
	...
	unmapped_addr =
		wifi_nrf_bal_dma_unmap(hal_dev_ctx->bal_dev_ctx, rx_buf_info->phy_addr,
				       rx_buf_info->buf_len, WIFI_NRF_OSAL_DMA_DIR_FROM_DEV);

	if (data_len) {
		if (!unmapped_addr) {
			wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv, "%s: DMA unmap failed\n",
					      __func__);
			goto out;
		}
		...
		wifi_nrf_bal_read_block(hal_dev_ctx->bal_dev_ctx,
					...
					unmapped_addr, data_len);
	}

	virt_addr = rx_buf_info->virt_addr;

	wifi_nrf_osal_mem_set(hal_dev_ctx->hpriv->opriv, rx_buf_info, 0, sizeof(*rx_buf_info));
out:
	return virt_addr;
}

unsigned long wifi_nrf_hal_buf_map_tx(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx, unsigned long buf,
				      unsigned int buf_len, unsigned int desc_id)
{
	...
	tx_buf_info->phy_addr = wifi_nrf_bal_dma_map(hal_dev_ctx->bal_dev_ctx, addr_to_map, buf_len,
					...
					unsigned int desc_id)
{
	...
	unmapped_addr = wifi_nrf_bal_dma_unmap(hal_dev_ctx->bal_dev_ctx, tx_buf_info->phy_addr,
					       tx_buf_info->buf_len, WIFI_NRF_OSAL_DMA_DIR_TO_DEV);
	...
	wifi_nrf_osal_mem_set(hal_dev_ctx->hpriv->opriv, tx_buf_info, 0, sizeof(*tx_buf_info));
out:
	return virt_addr;
}
...
enum wifi_nrf_status hal_rpu_ps_wake(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx)
{
	...
	do {
		/* Poll the RPU PS state register */
		reg_val = wifi_nrf_bal_rpu_ps_status(hal_dev_ctx->bal_dev_ctx);

		if (reg_val & rpu_ps_state_mask) {
			status = WIFI_NRF_STATUS_SUCCESS;
			break;
		}
...
static bool hal_rpu_hpq_is_empty(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx, struct host_rpu_hpq *hpq)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	unsigned int val = 0;

	status = hal_rpu_reg_read(hal_dev_ctx, &val, hpq->dequeue_addr);
					  ...
					  enum WIFI_NRF_HAL_MSG_TYPE msg_type)
{
	...
	if (msg_type == WIFI_NRF_HAL_MSG_TYPE_CMD_CTRL) {
		...
		wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv, "%s: Invalid msg type %d\n",
				      __func__, msg_type);
	...
	is_empty = hal_rpu_hpq_is_empty(hal_dev_ctx, avl_buf_q);
					       ...
					       enum WIFI_NRF_HAL_MSG_TYPE msg_type)
{
	...
	while (hal_rpu_ready(hal_dev_ctx, msg_type) != WIFI_NRF_STATUS_SUCCESS) {
		if (wifi_nrf_osal_time_elapsed_us(hal_dev_ctx->hpriv->opriv, start_time_us) >=
		    MAX_HAL_RPU_READY_WAIT) {
			wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv,
...
static enum wifi_nrf_status hal_rpu_msg_trigger(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx)
{
	...
	status = hal_rpu_reg_write(hal_dev_ctx, RPU_REG_INT_TO_MCU_CTRL,
...
static enum wifi_nrf_status hal_rpu_msg_post(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx,
					     enum WIFI_NRF_HAL_MSG_TYPE msg_type,
					     unsigned int queue_id, unsigned int msg_addr)
{
	...
	if ((msg_type == WIFI_NRF_HAL_MSG_TYPE_CMD_CTRL) ||
		...
		wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv, "%s: Invalid msg_type (%d)\n",
				      __func__, msg_type);
		goto out;
	}
	 ...
	 * to the busy queue.
	 */
	status = hal_rpu_hpq_enqueue(hal_dev_ctx, busy_queue, msg_addr);
						 ...
						 unsigned int *msg_addr)
{
	...
	status = hal_rpu_hpq_dequeue(hal_dev_ctx, avl_queue, msg_addr);
...
static enum wifi_nrf_status hal_rpu_msg_write(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx,
					      enum WIFI_NRF_HAL_MSG_TYPE msg_type, void *msg,
					      unsigned int len)
{
	 ...
	 */
	status = hal_rpu_msg_get_addr(hal_dev_ctx, msg_type, &msg_addr);
	...
	status = hal_rpu_mem_write(hal_dev_ctx, msg_addr, msg, len);
	...
	status = hal_rpu_msg_post(hal_dev_ctx, msg_type, 0, msg_addr);
...
static enum wifi_nrf_status hal_rpu_cmd_process_queue(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx)
{
	...
	while ((cmd = wifi_nrf_utils_q_dequeue(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->cmd_q))) {
		status = hal_rpu_ready_wait(hal_dev_ctx, WIFI_NRF_HAL_MSG_TYPE_CMD_CTRL);
		...
		if (status != WIFI_NRF_STATUS_SUCCESS) {
			wifi_nrf_osal_log_err(
				...
				__func__);
			wifi_nrf_osal_mem_free(hal_dev_ctx->hpriv->opriv, cmd);
			cmd = NULL;
			continue;
		}

		status = hal_rpu_msg_write(hal_dev_ctx, WIFI_NRF_HAL_MSG_TYPE_CMD_CTRL, cmd->data,
					   cmd->len);
...
static enum wifi_nrf_status hal_rpu_cmd_queue(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx, void *cmd,
					      unsigned int cmd_size)
{
	...
	if (len > hal_dev_ctx->hpriv->cfg_params.max_cmd_size) {
		while (len > 0) {
			...
			wifi_nrf_osal_mem_cpy(hal_dev_ctx->hpriv->opriv, hal_msg->data, data, size);

			hal_msg->len = size;

			status = wifi_nrf_utils_q_enqueue(hal_dev_ctx->hpriv->opriv,
							  hal_dev_ctx->cmd_q, hal_msg);
		...
		hal_msg =
			wifi_nrf_osal_mem_zalloc(hal_dev_ctx->hpriv->opriv, sizeof(*hal_msg) + len);
		...
		wifi_nrf_osal_mem_cpy(hal_dev_ctx->hpriv->opriv, hal_msg->data, cmd, len);

		hal_msg->len = len;

		status = wifi_nrf_utils_q_enqueue(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->cmd_q,
						  hal_msg);
...
enum wifi_nrf_status wifi_nrf_hal_ctrl_cmd_send(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx, void *cmd,
						unsigned int cmd_size)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;

	wifi_nrf_osal_spinlock_take(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->lock_hal);

	status = hal_rpu_cmd_queue(hal_dev_ctx, cmd, cmd_size);

	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv, "%s: Queueing of command failed\n",
				      __func__);
		goto out;
	}
...
out:
	wifi_nrf_osal_spinlock_rel(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->lock_hal);

	return status;
}

enum wifi_nrf_status wifi_nrf_hal_data_cmd_send(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx,
						enum WIFI_NRF_HAL_MSG_TYPE cmd_type, void *cmd,
						unsigned int cmd_size, unsigned int desc_id,
						unsigned int pool_id)
{
	...
	/* Copy the information to the suggested address */
	status = hal_rpu_mem_write(hal_dev_ctx, addr, cmd, cmd_size);
	...
	/* Post the updated information to the RPU */
	status = hal_rpu_msg_post(hal_dev_ctx, cmd_type, pool_id, addr);
...
static enum wifi_nrf_status hal_rpu_eventq_process(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx)
{
	...
	while (1) {
		wifi_nrf_osal_spinlock_irq_take(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->lock_rx,
						&flags);

		event = wifi_nrf_utils_q_dequeue(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->event_q);

		wifi_nrf_osal_spinlock_irq_rel(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->lock_rx,
					       &flags);
		...
		status = hal_dev_ctx->hpriv->intr_callbk_fn(hal_dev_ctx->mac_dev_ctx, event_data,
							    event_len);
		...
		wifi_nrf_osal_mem_free(hal_dev_ctx->hpriv->opriv, event);
		event = NULL;
	}
						  ...
						  void *mac_dev_ctx)
{
	...
	hal_dev_ctx = wifi_nrf_osal_mem_zalloc(hpriv->opriv, sizeof(*hal_dev_ctx));

	if (!hal_dev_ctx) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: Unable to allocate hal_dev_ctx\n",
				      __func__);
		goto out;
	}
	...
	if (!hal_dev_ctx->cmd_q) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: Unable to allocate command queue\n",
				      __func__);
	...
	if (!hal_dev_ctx->event_q) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: Unable to allocate event queue\n",
				      __func__);
	...
	if (!hal_dev_ctx->lock_hal) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: Unable to allocate HAL lock\n", __func__);
		wifi_nrf_utils_q_free(hpriv->opriv, hal_dev_ctx->cmd_q);
	...
	wifi_nrf_osal_spinlock_init(hpriv->opriv, hal_dev_ctx->lock_hal);
	...
	wifi_nrf_osal_spinlock_init(hpriv->opriv, hal_dev_ctx->lock_rx);

	hal_dev_ctx->rx_tasklet = wifi_nrf_osal_tasklet_alloc(hpriv->opriv);

	if (!hal_dev_ctx->rx_tasklet) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: Unable to allocate rx_tasklet\n",
				      __func__);
		wifi_nrf_osal_spinlock_free(hpriv->opriv, hal_dev_ctx->lock_hal);
	...
	wifi_nrf_osal_tasklet_init(hpriv->opriv, hal_dev_ctx->rx_tasklet, rx_tasklet_fn,
	...
	hal_dev_ctx->bal_dev_ctx = wifi_nrf_bal_dev_add(hpriv->bpriv, hal_dev_ctx);

	if (!hal_dev_ctx->bal_dev_ctx) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: wifi_nrf_bal_dev_add failed\n", __func__);
	...
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: hal_rpu_irq_enable failed\n", __func__);
	...
	for (i = 0; i < MAX_NUM_OF_RX_QUEUES; i++) {
		...
		hal_dev_ctx->rx_buf_info[i] =
			(struct wifi_nrf_hal_buf_map_info *)wifi_nrf_osal_mem_zalloc(hpriv->opriv,
										     size);

		if (!hal_dev_ctx->rx_buf_info[i]) {
			wifi_nrf_osal_log_err(hpriv->opriv, "%s: No space for RX buf info[%d]\n",
					      __func__, i);
	...
	hal_dev_ctx->tx_buf_info =
		(struct wifi_nrf_hal_buf_map_info *)wifi_nrf_osal_mem_zalloc(hpriv->opriv, size);

	if (!hal_dev_ctx->tx_buf_info) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: No space for TX buf info\n", __func__);

		for (i = 0; i < MAX_NUM_OF_RX_QUEUES; i++) {
			wifi_nrf_osal_mem_free(hpriv->opriv, hal_dev_ctx->rx_buf_info[i]);
	...
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(hpriv->opriv, "%s: Buffer map init failed\n", __func__);
		wifi_nrf_osal_mem_free(hpriv->opriv, hal_dev_ctx->tx_buf_info);
		...
		wifi_nrf_utils_q_free(hpriv->opriv, hal_dev_ctx->cmd_q);
		wifi_nrf_utils_q_free(hpriv->opriv, hal_dev_ctx->event_q);
		wifi_nrf_osal_mem_free(hpriv->opriv, hal_dev_ctx->rx_buf_info);
		wifi_nrf_osal_mem_free(hpriv->opriv, hal_dev_ctx);
		hal_dev_ctx = NULL;
		goto out;
	}
...
void wifi_nrf_hal_dev_rem(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx)
{
	...
	wifi_nrf_osal_mem_free(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->tx_buf_info);
	hal_dev_ctx->tx_buf_info = NULL;

	for (i = 0; i < MAX_NUM_OF_RX_QUEUES; i++) {
		wifi_nrf_osal_mem_free(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->rx_buf_info[i]);
		hal_dev_ctx->rx_buf_info[i] = NULL;
	}

	wifi_nrf_osal_tasklet_kill(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->rx_tasklet);

	wifi_nrf_osal_tasklet_free(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->rx_tasklet);

	wifi_nrf_osal_spinlock_free(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->lock_hal);
	wifi_nrf_osal_spinlock_free(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->lock_rx);

	wifi_nrf_utils_q_free(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->event_q);

	wifi_nrf_utils_q_free(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->cmd_q);
...
enum wifi_nrf_status wifi_nrf_hal_dev_init(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx)
{
	 ...
	 */
	status = hal_rpu_mem_read(hal_dev_ctx, &hal_dev_ctx->rpu_info.hpqm_info, RPU_MEM_HPQ_INFO,
				  sizeof(hal_dev_ctx->rpu_info.hpqm_info));

	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv, "%s: Failed to get the HPQ info\n",
				      __func__);
		goto out;
	}

	status = hal_rpu_mem_read(hal_dev_ctx, &hal_dev_ctx->rpu_info.rx_cmd_base,
				  RPU_MEM_RX_CMD_BASE, sizeof(hal_dev_ctx->rpu_info.rx_cmd_base));
...
enum wifi_nrf_status wifi_nrf_hal_irq_handler(void *data)
{
	...
	wifi_nrf_osal_spinlock_irq_take(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->lock_rx, &flags);
	...
	wifi_nrf_osal_tasklet_schedule(hal_dev_ctx->hpriv->opriv, hal_dev_ctx->rx_tasklet);

out:
	return status;
}

static int wifi_nrf_hal_poll_reg(struct wifi_nrf_hal_dev_ctx *hal_dev_ctx, unsigned int reg_addr,
				 unsigned int mask, unsigned int req_value, unsigned int poll_delay)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	unsigned int val = 0;
	...
	do {
		status = hal_rpu_reg_read(hal_dev_ctx, &val, reg_addr);
		...
		wifi_nrf_osal_sleep_ms(hal_dev_ctx->hpriv->opriv, poll_delay);
	} while (count-- > 0);
					     ...
					     enum RPU_PROC_TYPE rpu_proc)
{
	...
	if ((rpu_proc != RPU_PROC_TYPE_MCU_LMAC) && (rpu_proc != RPU_PROC_TYPE_MCU_UMAC)) {
		wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv,
	...
	if (rpu_proc == RPU_PROC_TYPE_MCU_LMAC) {
		status = hal_rpu_reg_write(hal_dev_ctx, RPU_REG_MIPS_MCU_CONTROL, 0x1);
	} else {
		status = hal_rpu_reg_write(hal_dev_ctx, RPU_REG_MIPS_MCU2_CONTROL, 0x1);
	}
	...
	if (rpu_proc == RPU_PROC_TYPE_MCU_LMAC) {
		status = wifi_nrf_hal_poll_reg(hal_dev_ctx, RPU_REG_MIPS_MCU_CONTROL, 0x1, 0, 10);
	} else {
		status = wifi_nrf_hal_poll_reg(hal_dev_ctx, RPU_REG_MIPS_MCU2_CONTROL, 0x1, 0, 10);
	}
	 ...
	 */
	if (rpu_proc == RPU_PROC_TYPE_MCU_LMAC) {
		status = wifi_nrf_hal_poll_reg(hal_dev_ctx, 0xA4000018, 0x1, 0x1, 10);
	} else {
		status = wifi_nrf_hal_poll_reg(hal_dev_ctx, 0xA4000118, 0x1, 0x1, 10);
	}
out:
	return status;
}
					      ...
					      enum RPU_PROC_TYPE rpu_proc)
{
	...
	if (rpu_proc == RPU_PROC_TYPE_MCU_LMAC) {
		...
		wifi_nrf_osal_log_err(hal_dev_ctx->hpriv->opriv, "%s: Invalid RPU processor (%d)\n",
				      __func__, rpu_proc);
	}

	while (i < 1000) {
		status = hal_rpu_mem_read(hal_dev_ctx, (unsigned char *)&val, addr, sizeof(val));
		...
		/* Sleep for 10 ms */
		wifi_nrf_osal_sleep_ms(hal_dev_ctx->hpriv->opriv, 10);
...
wifi_nrf_hal_init(struct wifi_nrf_osal_priv *opriv, struct wifi_nrf_hal_cfg_params *cfg_params,
		  enum wifi_nrf_status (*intr_callbk_fn)(void *mac_dev_ctx, void *event_data,
							 unsigned int len))
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	...
	hpriv = wifi_nrf_osal_mem_zalloc(opriv, sizeof(*hpriv));

	if (!hpriv) {
		wifi_nrf_osal_log_err(opriv, "%s: Unable to allocate memory for hpriv\n", __func__);
		goto out;
	}

	hpriv->opriv = opriv;

	wifi_nrf_osal_mem_cpy(opriv, &hpriv->cfg_params, cfg_params, sizeof(hpriv->cfg_params));

	hpriv->intr_callbk_fn = intr_callbk_fn;

	status = pal_rpu_addr_offset_get(opriv, RPU_ADDR_PKTRAM_START, &hpriv->addr_pktram_base);

	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(opriv, "%s: pal_rpu_addr_offset_get failed\n", __func__);
		goto out;
	}

	bal_cfg_params.addr_pktram_base = hpriv->addr_pktram_base;

	hpriv->bpriv = wifi_nrf_bal_init(opriv, &bal_cfg_params, &wifi_nrf_hal_irq_handler);

	if (!hpriv->bpriv) {
		wifi_nrf_osal_log_err(opriv, "%s: Failed\n", __func__);
		wifi_nrf_osal_mem_free(opriv, hpriv);
		hpriv = NULL;
	}
out:
	return hpriv;
...
void wifi_nrf_hal_deinit(struct wifi_nrf_hal_priv *hpriv)
{
	...
	wifi_nrf_osal_mem_free(hpriv->opriv, hpriv);
}
