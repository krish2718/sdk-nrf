...
enum wifi_nrf_status hal_rpu_hpq_enqueue(struct wifi_nrf_hal_dev_ctx *hal_ctx,
					 struct host_rpu_hpq *hpq, unsigned int val)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;

	status = hal_rpu_reg_write(hal_ctx, hpq->enqueue_addr, val);
					 ...
					 struct host_rpu_hpq *hpq, unsigned int *val)
{
	...
	status = hal_rpu_reg_read(hal_ctx, val, hpq->dequeue_addr);

	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(hal_ctx->hpriv->opriv, "%s: Dequeue failed, val (0x%X)\n",
				      __func__, *val);
		goto out;
	}
	...
	if (*val) {
		status = hal_rpu_reg_write(hal_ctx, hpq->dequeue_addr, *val);
