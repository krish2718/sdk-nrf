				...
				struct wpa_bss *curr_bss)
{
	...
	auth_info.local_state_change = params->local_state_change;
				     ...
				     const unsigned char *addr, int key_idx, uint32_t db_id)
{
	...
	/* Pad/copy up to max size for type: 16 bytes for MIC, 32 for ENC. Pad with zeros if shorter */
	if (key && key_len > 0) {
		if (key_len >= max_size) {
...
int nrf_wifi_supp_get_capa(void *if_priv, struct wpa_driver_capa *capa)
{
	...
	if (IS_ENABLED(CONFIG_NRF71_P2P_MODE)) {
