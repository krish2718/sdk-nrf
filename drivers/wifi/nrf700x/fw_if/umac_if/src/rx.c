...
void wifi_nrf_scan_result(struct wifi_nrf_fmac_vif_ctx *wifi_nrf_vif_ctx, void *nwb,
			  struct wifi_nrf_rx_buff *config);
#endif

static enum wifi_nrf_status
wifi_nrf_fmac_map_desc_to_pool(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx, unsigned int desc_id,
					       ...
					       unsigned int desc_id)
{
	...
	status = wifi_nrf_fmac_map_desc_to_pool(fmac_dev_ctx, desc_id, &pool_info);
	...
	if (cmd_type == WIFI_NRF_FMAC_RX_CMD_TYPE_INIT) {
		...
		nwb = (unsigned long)wifi_nrf_osal_nbuf_alloc(fmac_dev_ctx->fpriv->opriv, buf_len);
		...
		*(unsigned int *)(nwb_data) = desc_id;
		...
		phy_addr = wifi_nrf_hal_buf_map_rx(fmac_dev_ctx->hal_dev_ctx, nwb_data, buf_len,
						   pool_info.pool_id, pool_info.buf_id);
		...
		wifi_nrf_osal_mem_set(fmac_dev_ctx->fpriv->opriv, &rx_cmd, 0x0, sizeof(rx_cmd));
		...
		status = wifi_nrf_hal_data_cmd_send(fmac_dev_ctx->hal_dev_ctx,
						    WIFI_NRF_HAL_MSG_TYPE_CMD_DATA_RX, &rx_cmd,
						    sizeof(rx_cmd), desc_id, pool_info.pool_id);
		...
		nwb_data = wifi_nrf_hal_buf_unmap_rx(fmac_dev_ctx->hal_dev_ctx, 0,
						     pool_info.pool_id, pool_info.buf_id);
		...
		wifi_nrf_osal_nbuf_free(fmac_dev_ctx->fpriv->opriv, (void *)rx_buf_info->nwb);
		rx_buf_info->nwb = 0;
		rx_buf_info->mapped = false;
		status = WIFI_NRF_STATUS_SUCCESS;
	} else {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Unknown cmd_type (%d)\n",
				      __func__, cmd_type);
		goto out;
	}
out:
						    ...
						    struct img_rx_buff *config)
{
	...
	for (i = 0; i < num_pkts; i++) {
		...
		nwb_data = (void *)wifi_nrf_hal_buf_unmap_rx(fmac_dev_ctx->hal_dev_ctx, pkt_len,
							     pool_info.pool_id, pool_info.buf_id);
		...
		wifi_nrf_osal_nbuf_data_put(fmac_dev_ctx->fpriv->opriv, nwb,
					    pkt_len + RX_BUF_HEADROOM);
		wifi_nrf_osal_nbuf_data_pull(fmac_dev_ctx->fpriv->opriv, nwb, RX_BUF_HEADROOM);
		nwb_data = wifi_nrf_osal_nbuf_data_get(fmac_dev_ctx->fpriv->opriv, nwb);
		...
		if (config->rx_pkt_type == IMG_RX_PKT_DATA) {
			...
			case PKT_TYPE_MPDU:
				wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &hdr, nwb_data,
						      sizeof(struct wifi_nrf_fmac_ieee80211_hdr));

				eth_type = wifi_nrf_util_rx_get_eth_type(
					fmac_dev_ctx,
					(void *)((char *)nwb_data + config->mac_header_len));
				...
				wifi_nrf_util_convert_to_eth(fmac_dev_ctx, nwb, &hdr, eth_type);
				break;
			case PKT_TYPE_MSDU_WITH_MAC:
				wifi_nrf_osal_nbuf_data_pull(fmac_dev_ctx->fpriv->opriv, nwb,
							     config->mac_header_len);

				wifi_nrf_util_rx_convert_amsdu_to_eth(fmac_dev_ctx, nwb);
				break;
			case PKT_TYPE_MSDU:
				wifi_nrf_util_rx_convert_amsdu_to_eth(fmac_dev_ctx, nwb);
				break;
			...
			fmac_dev_ctx->fpriv->callbk_fns.rx_frm_callbk_fn(vif_ctx->os_vif_ctx, nwb);
		} else if (config->rx_pkt_type == IMG_RX_PKT_BCN_PRB_RSP) {
#ifndef RPU_BSS_DB_SUPPORT
			wifi_nrf_scan_result(vif_ctx, nwb, config);
#endif
