...
static unsigned char wifi_nrf_fmac_vif_idx_get(struct wifi_nrf_fmac_dev_ctx *wifi_nrf_fmac_dev_ctx)
{
	...
	for (i = 0; i < MAX_NUM_VIFS; i++) {
		if (wifi_nrf_fmac_dev_ctx->vif_ctx[i] == NULL) {
			break;
		}
	}
...
static enum wifi_nrf_status wifi_nrf_fmac_init_tx(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx)
{
	...
	size = (fpriv->num_tx_tokens * fpriv->data_config.max_tx_aggregation *
		sizeof(struct wifi_nrf_fmac_buf_map_info));

	fmac_dev_ctx->tx_buf_info = (struct wifi_nrf_fmac_buf_map_info *)wifi_nrf_osal_mem_zalloc(
		fmac_dev_ctx->fpriv->opriv, size);

	if (!fmac_dev_ctx->tx_buf_info) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: No space for TX buf info\n",
				      __func__);
		goto out;
	}
...
static void wifi_nrf_fmac_deinit_tx(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx)
{
	...
	wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, fmac_dev_ctx->tx_buf_info);
}
...
static enum wifi_nrf_status wifi_nrf_fmac_init_rx(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx)
{
	...
	fmac_dev_ctx->rx_buf_info = (struct wifi_nrf_fmac_buf_map_info *)wifi_nrf_osal_mem_zalloc(
		fmac_dev_ctx->fpriv->opriv, size);

	if (!fmac_dev_ctx->rx_buf_info) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: No space for RX buf info\n",
				      __func__);
		goto out;
	}

	for (desc_id = 0; desc_id < fmac_dev_ctx->fpriv->num_rx_bufs; desc_id++) {
		status = wifi_nrf_fmac_rx_cmd_send(fmac_dev_ctx, WIFI_NRF_FMAC_RX_CMD_TYPE_INIT,
						   desc_id);
...
static enum wifi_nrf_status wifi_nrf_fmac_deinit_rx(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx)
{
	...
	for (desc_id = 0; desc_id < fpriv->num_rx_bufs; desc_id++) {
		status = wifi_nrf_fmac_rx_cmd_send(fmac_dev_ctx, WIFI_NRF_FMAC_RX_CMD_TYPE_DEINIT,
						   desc_id);
	...
	wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, fmac_dev_ctx->rx_buf_info);
						  ...
						  unsigned int phy_calib)
{
	...
	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, fmac_dev_ctx->base_mac_addr, mac,
			      IMG_ETH_ALEN);

	status = wifi_nrf_fmac_init_tx(fmac_dev_ctx);

	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Init TX failed\n", __func__);
		goto out;
	}
	...
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Init RX failed\n", __func__);
	...
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: UMAC init failed\n",
				      __func__);
...
#define MAX_INIT_WAIT (5 * 1000 * 1000)
		if (wifi_nrf_osal_time_elapsed_us(fmac_dev_ctx->fpriv->opriv, start_time_us) >=
		    MAX_INIT_WAIT) {
			break;
		}
	}
	...
	if (!fmac_dev_ctx->init_done) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: UMAC init timed out\n",
				      __func__);
		wifi_nrf_fmac_deinit_rx(fmac_dev_ctx);
		wifi_nrf_fmac_deinit_tx(fmac_dev_ctx);
...
static void wifi_nrf_fmac_fw_deinit(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx)
{
	...
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: UMAC deinit failed\n",
				      __func__);
		goto out;
	}
...
#define MAX_DEINIT_WAIT (5 * 1000 * 1000)
	...
	if (!fmac_dev_ctx->deinit_done) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: UMAC deinit timed out\n",
				      __func__);
		status = WIFI_NRF_STATUS_FAIL;
		goto out;
	}
						    ...
						    void *os_dev_ctx)
{
	...
	fmac_dev_ctx = wifi_nrf_osal_mem_zalloc(fpriv->opriv, sizeof(*fmac_dev_ctx));

	if (!fmac_dev_ctx) {
		wifi_nrf_osal_log_err(fpriv->opriv, "%s: Unable to allocate fmac_dev_ctx\n",
				      __func__);
		goto out;
	}
	...
	fmac_dev_ctx->hal_dev_ctx = wifi_nrf_hal_dev_add(fpriv->hpriv, fmac_dev_ctx);

	if (!fmac_dev_ctx->hal_dev_ctx) {
		wifi_nrf_osal_log_err(fpriv->opriv, "%s: wifi_nrf_hal_dev_add failed\n", __func__);

		wifi_nrf_osal_mem_free(fpriv->opriv, fmac_dev_ctx);
		fmac_dev_ctx = NULL;
		goto out;
	}
out:
					      ...
					      struct wifi_nrf_fmac_callbk_fns *callbk_fns)
{
	...
	fpriv = wifi_nrf_osal_mem_zalloc(opriv, sizeof(*fpriv));

	if (!fpriv) {
		wifi_nrf_osal_log_err(opriv, "%s: Unable to allocate fpriv\n", __func__);
		goto out;
	}

	fpriv->opriv = opriv;

	wifi_nrf_osal_mem_cpy(opriv, &fpriv->callbk_fns, callbk_fns, sizeof(fpriv->callbk_fns));

	wifi_nrf_osal_mem_cpy(opriv, &fpriv->data_config, data_config, sizeof(fpriv->data_config));
	...
	wifi_nrf_osal_mem_cpy(opriv, fpriv->rx_buf_pools, rx_buf_pools,
	...
	hal_cfg_params.max_tx_frms = (fpriv->num_tx_tokens * fpriv->data_config.max_tx_aggregation);
	...
	fpriv->hpriv = wifi_nrf_hal_init(opriv, &hal_cfg_params, &wifi_nrf_fmac_event_callback);

	if (!fpriv->hpriv) {
		wifi_nrf_osal_log_err(opriv, "%s: Unable to do HAL init\n", __func__);
		wifi_nrf_osal_mem_free(opriv, fpriv);
		fpriv = NULL;
					   ...
					   struct wifi_nrf_fmac_fw_info *fmac_fw)
{
	...
	if ((fmac_fw->lmac_patch_pri.data) && (fmac_fw->lmac_patch_pri.size)) {
		status = wifi_nrf_hal_proc_reset(fmac_dev_ctx->hal_dev_ctx, RPU_PROC_TYPE_MCU_LMAC);
		...
		status = wifi_nrf_hal_fw_patch_load(
			fmac_dev_ctx->hal_dev_ctx, RPU_PROC_TYPE_MCU_LMAC,
			fmac_fw->lmac_patch_pri.data, fmac_fw->lmac_patch_pri.size,
		...
		status =
			wifi_nrf_hal_fw_chk_boot(fmac_dev_ctx->hal_dev_ctx, RPU_PROC_TYPE_MCU_LMAC);
	...
	if ((fmac_fw->umac_patch_pri.data) && (fmac_fw->umac_patch_pri.size)) {
		status = wifi_nrf_hal_proc_reset(fmac_dev_ctx->hal_dev_ctx, RPU_PROC_TYPE_MCU_UMAC);
		...
		status = wifi_nrf_hal_fw_patch_load(
			fmac_dev_ctx->hal_dev_ctx, RPU_PROC_TYPE_MCU_UMAC,
			fmac_fw->umac_patch_pri.data, fmac_fw->umac_patch_pri.size,
		...
		status =
			wifi_nrf_hal_fw_chk_boot(fmac_dev_ctx->hal_dev_ctx, RPU_PROC_TYPE_MCU_UMAC);
					     ...
					     struct rpu_op_stats *stats)
{
	...
	if ((stats_type == RPU_STATS_TYPE_ALL) || (stats_type == RPU_STATS_TYPE_UMAC) ||
	    (stats_type == RPU_STATS_TYPE_LMAC) || (stats_type == RPU_STATS_TYPE_PHY)) {
		...
		status = umac_cmd_prog_stats_get(fmac_dev_ctx, stats_type);
		...
		if (count == WIFI_NRF_FMAC_STATS_RECV_TIMEOUT) {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Timed out\n",
					      __func__);
			goto out;
		}
	}

	if ((stats_type == RPU_STATS_TYPE_ALL) || (stats_type == RPU_STATS_TYPE_HOST)) {
		wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &stats->host,
				      &fmac_dev_ctx->host_stats, sizeof(fmac_dev_ctx->host_stats));
	}
...
enum wifi_nrf_status wifi_nrf_fmac_ver_get(struct wifi_nrf_fmac_dev_ctx *fmac_dev_ctx)
{
	...
	status = hal_rpu_mem_read(fmac_dev_ctx->hal_dev_ctx, &fmac_dev_ctx->umac_ver, umac_ver_addr,
				  sizeof(fmac_dev_ctx->umac_ver));

	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Unable to read UMAC ver\n",
				      __func__);
		goto out;
	}

	status = hal_rpu_mem_read(fmac_dev_ctx->hal_dev_ctx, &fmac_dev_ctx->lmac_ver, lmac_ver_addr,
				  sizeof(fmac_dev_ctx->lmac_ver));

	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Unable to read LMAC ver\n",
				      __func__);
		goto out;
	}
...
enum wifi_nrf_status wifi_nrf_fmac_scan(void *wifi_nrf_fmac_dev_ctx, unsigned char wifi_nrf_vif_idx,
					struct img_umac_scan_info *scan_info)
{
	...
	int channel_info_len =
		(sizeof(struct img_channel) * scan_info->scan_params.num_scan_channels);
	...
	scan_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	scan_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &scan_cmd->info, scan_info,
			      (sizeof(scan_cmd->info) + channel_info_len));

	status = umac_cmd_cfg(fmac_dev_ctx, scan_cmd, sizeof(*scan_cmd) + channel_info_len);
out:
	if (scan_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, scan_cmd);
	}

	return status;
}

enum wifi_nrf_status wifi_nrf_fmac_scan_res_get(void *wifi_nrf_fmac_dev_ctx, unsigned char vif_idx,
						int scan_type)

{
	...
	scan_res_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*scan_res_cmd));
	...
	status = umac_cmd_cfg(fmac_dev_ctx, scan_res_cmd, sizeof(*scan_res_cmd));
out:
	if (scan_res_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, scan_res_cmd);
	}

	return status;
}

enum wifi_nrf_status wifi_nrf_fmac_auth(void *wifi_nrf_fmac_dev_ctx, unsigned char wifi_nrf_vif_idx,
					struct img_umac_auth_info *auth_info)
{
	...
	auth_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*auth_cmd));
	...
	auth_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	auth_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &auth_cmd->info, auth_info,
	...
	status = umac_cmd_cfg(fmac_dev_ctx, auth_cmd, sizeof(*auth_cmd));
out:
	if (auth_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, auth_cmd);
	}
					  ...
					  struct img_umac_disconn_info *deauth_info)
{
	...
	deauth_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*deauth_cmd));
	...
	deauth_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	deauth_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &deauth_cmd->info, deauth_info,
			      sizeof(deauth_cmd->info));

	if (!wifi_nrf_util_is_arr_zero(deauth_info->mac_addr, sizeof(deauth_info->mac_addr))) {
		deauth_cmd->valid_fields |= IMG_CMD_MLME_MAC_ADDR_VALID;
	}

	status = umac_cmd_cfg(fmac_dev_ctx, deauth_cmd, sizeof(*deauth_cmd));

out:
	if (deauth_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, deauth_cmd);
	}
					 ...
					 struct img_umac_assoc_info *assoc_info)
{
	...
	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, vif_ctx->bssid, assoc_info->img_bssid,
			      IMG_ETH_ALEN);

	assoc_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*assoc_cmd));
	...
	assoc_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	assoc_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	connect_common_info = &assoc_cmd->connect_common_info;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, connect_common_info->mac_addr,
			      assoc_info->img_bssid, IMG_ETH_ALEN);

	if (!wifi_nrf_util_is_arr_zero(connect_common_info->mac_addr,
				       sizeof(connect_common_info->mac_addr))) {
		connect_common_info->valid_fields |= IMG_CONNECT_COMMON_INFO_MAC_ADDR_VALID;
	}

	if (assoc_info->ssid.img_ssid_len > 0) {
		wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &connect_common_info->ssid,
				      &assoc_info->ssid, sizeof(connect_common_info->ssid));

		connect_common_info->valid_fields |= IMG_CONNECT_COMMON_INFO_SSID_VALID;
	}
	...
	if (assoc_info->wpa_ie.ie_len > 0) {
		wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &connect_common_info->wpa_ie,
				      &assoc_info->wpa_ie, sizeof(connect_common_info->wpa_ie));

		connect_common_info->valid_fields |= IMG_CONNECT_COMMON_INFO_WPA_IE_VALID;
	}

	connect_common_info->use_mfp = assoc_info->use_mfp;
	connect_common_info->valid_fields |= IMG_CONNECT_COMMON_INFO_USE_MFP_VALID;

	connect_common_info->control_port = assoc_info->control_port;

	status = umac_cmd_cfg(fmac_dev_ctx, assoc_cmd, sizeof(*assoc_cmd));
out:
	if (assoc_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, assoc_cmd);
	}
					    ...
					    struct img_umac_disconn_info *disassoc_info)
{
	...
	disassoc_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*disassoc_cmd));
	...
	disassoc_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	disassoc_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &disassoc_cmd->info, disassoc_info,
			      sizeof(disassoc_cmd->info));

	if (!wifi_nrf_util_is_arr_zero(disassoc_info->mac_addr, sizeof(disassoc_info->mac_addr))) {
		disassoc_cmd->valid_fields |= IMG_CMD_MLME_MAC_ADDR_VALID;
	}

	status = umac_cmd_cfg(fmac_dev_ctx, disassoc_cmd, sizeof(*disassoc_cmd));

out:
	if (disassoc_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, disassoc_cmd);
	}
...
enum wifi_nrf_status wifi_nrf_fmac_add_key(void *wifi_nrf_fmac_dev_ctx,
					   unsigned char wifi_nrf_vif_idx,
					   struct img_umac_key_info *key_info, const char *mac_addr)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	...
	key_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*key_cmd));
	...
	key_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	key_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &key_cmd->key_info, key_info,
			      sizeof(key_cmd->key_info));

	if (mac_addr) {
		wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, key_cmd->mac_addr, mac_addr,
				      IMG_ETH_ALEN);
	...
	if (key_info->key_type == IMG_KEYTYPE_GROUP) {
		...
		peer_id = wifi_nrf_fmac_peer_get_id(fmac_dev_ctx, mac_addr);

		if (peer_id == -1) {
			wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Invalid peer\n",
					      __func__);
			goto out;
		}
		...
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: Invalid key type %d\n",
				      __func__, key_info->key_type);
		goto out;
	}
	...
	status = umac_cmd_cfg(fmac_dev_ctx, key_cmd, sizeof(*key_cmd));

out:
	if (key_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, key_cmd);
	}
					   ...
					   struct img_umac_key_info *key_info)
{
	...
	set_key_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*set_key_cmd));
	...
	set_key_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	set_key_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &set_key_cmd->key_info, key_info,
			      sizeof(set_key_cmd->key_info));

	set_key_cmd->key_info.valid_fields |= IMG_KEY_IDX_VALID;

	status = umac_cmd_cfg(fmac_dev_ctx, set_key_cmd, sizeof(*set_key_cmd));

out:
	if (set_key_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, set_key_cmd);
	}
					   ...
					   struct img_umac_bss_info *bss_info)
{
	...
	set_bss_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*set_bss_cmd));
	...
	set_bss_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	set_bss_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &set_bss_cmd->bss_info, bss_info,
			      sizeof(set_bss_cmd->bss_info));

	set_bss_cmd->valid_fields = IMG_CMD_SET_BSS_CTS_VALID | IMG_CMD_SET_BSS_PREAMBLE_VALID |
				    IMG_CMD_SET_BSS_SLOT_VALID | IMG_CMD_SET_BSS_HT_OPMODE_VALID |
				    IMG_CMD_SET_BSS_AP_ISOLATE_VALID;

	if ((bss_info->p2p_go_ctwindow > 0) && (bss_info->p2p_go_ctwindow < 127)) {
		set_bss_cmd->valid_fields |=
			(IMG_CMD_SET_BSS_P2P_CTWINDOW_VALID | IMG_CMD_SET_BSS_P2P_OPPPS_VALID);
	}

	status = umac_cmd_cfg(fmac_dev_ctx, set_bss_cmd, sizeof(*set_bss_cmd));

out:
	if (set_bss_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, set_bss_cmd);
	}
					   ...
					   struct img_umac_set_beacon_info *data)
{
	...
	set_bcn_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*set_bcn_cmd));
	...
	set_bcn_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	set_bcn_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &set_bcn_cmd->info, data,
			      sizeof(set_bcn_cmd->info));

	wifi_nrf_osal_log_dbg(fmac_dev_ctx->fpriv->opriv, "%s: Sending command to rpu\n", __func__);

	status = umac_cmd_cfg(fmac_dev_ctx, set_bcn_cmd, sizeof(*set_bcn_cmd));

out:
	if (set_bcn_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, set_bcn_cmd);
	}
			       ...
			       struct img_umac_set_wiphy_info *wiphy_info)
{
	...
	set_wiphy_cmd =
		wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*set_wiphy_cmd));
	...
	set_wiphy_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	set_wiphy_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	if (wiphy_info->freq_params.frequency) {
		freq_params_valid = 1;
		wiphy_info->freq_params.valid_fields |= IMG_SET_FREQ_PARAMS_FREQ_VALID;
	}
	if (wiphy_info->freq_params.channel_width) {
		freq_params_valid = 1;
		wiphy_info->freq_params.valid_fields |= IMG_SET_FREQ_PARAMS_CHANNEL_WIDTH_VALID;
	}
	if (wiphy_info->freq_params.center_frequency1) {
		freq_params_valid = 1;
		wiphy_info->freq_params.valid_fields |= IMG_SET_FREQ_PARAMS_CENTER_FREQ1_VALID;
	}
	if (wiphy_info->freq_params.center_frequency2) {
		freq_params_valid = 1;
		wiphy_info->freq_params.valid_fields |= IMG_SET_FREQ_PARAMS_CENTER_FREQ2_VALID;
	}
	if (wiphy_info->freq_params.channel_type) {
		freq_params_valid = 1;
		wiphy_info->freq_params.valid_fields |= IMG_SET_FREQ_PARAMS_CHANNEL_TYPE_VALID;
	}

	if (freq_params_valid) {
		set_wiphy_cmd->valid_fields |= IMG_CMD_SET_WIPHY_FREQ_PARAMS_VALID;
	}

	if (wiphy_info->rts_threshold) {
		set_wiphy_cmd->valid_fields |= IMG_CMD_SET_WIPHY_RTS_THRESHOLD_VALID;
	}

	if (wiphy_info->frag_threshold) {
		set_wiphy_cmd->valid_fields |= IMG_CMD_SET_WIPHY_FRAG_THRESHOLD_VALID;
	}

	if (wiphy_info->retry_long) {
		set_wiphy_cmd->valid_fields |= IMG_CMD_SET_WIPHY_RETRY_LONG_VALID;
	}

	if (wiphy_info->retry_short) {
		set_wiphy_cmd->valid_fields |= IMG_CMD_SET_WIPHY_RETRY_SHORT_VALID;
	}

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &set_wiphy_cmd->info, wiphy_info,
			      sizeof(set_wiphy_cmd->info));

	status = umac_cmd_cfg(fmac_dev_ctx, set_wiphy_cmd, sizeof(*set_wiphy_cmd));
out:
	if (set_wiphy_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, set_wiphy_cmd);
	}
					    ...
					    struct img_umac_start_ap_info *ap_info)
{
	...
	start_ap_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*start_ap_cmd));
	...
	start_ap_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	start_ap_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &start_ap_cmd->info, ap_info,
			      sizeof(start_ap_cmd->info));

	start_ap_cmd->valid_fields |= IMG_CMD_BEACON_INFO_BEACON_INTERVAL_VALID |
				      IMG_CMD_BEACON_INFO_VERSIONS_VALID |
				      IMG_CMD_BEACON_INFO_CIPHER_SUITE_GROUP_VALID;

	start_ap_cmd->info.freq_params.valid_fields |=
		IMG_SET_FREQ_PARAMS_FREQ_VALID | IMG_SET_FREQ_PARAMS_CHANNEL_WIDTH_VALID |
		IMG_SET_FREQ_PARAMS_CENTER_FREQ1_VALID | IMG_SET_FREQ_PARAMS_CENTER_FREQ2_VALID |
	...
	if ((ap_info->p2p_go_ctwindow > 0) && (ap_info->p2p_go_ctwindow < 127)) {
		start_ap_cmd->valid_fields |= IMG_CMD_BEACON_INFO_P2P_CTWINDOW_VALID;
		start_ap_cmd->valid_fields |= IMG_CMD_BEACON_INFO_P2P_OPPPS_VALID;
	}

	wiphy_info = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*wiphy_info));
	...
	status = wifi_nrf_fmac_set_wiphy_params(fmac_dev_ctx, wifi_nrf_vif_idx, wiphy_info);
	...
	status = umac_cmd_cfg(fmac_dev_ctx, start_ap_cmd, sizeof(*start_ap_cmd));

out:
	if (wiphy_info) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, wiphy_info);
	}

	if (start_ap_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, start_ap_cmd);
	}
					   ...
					   unsigned char wifi_nrf_vif_idx)
{
	...
	stop_ap_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*stop_ap_cmd));
	...
	status = umac_cmd_cfg(fmac_dev_ctx, stop_ap_cmd, sizeof(*stop_ap_cmd));

out:
	if (stop_ap_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, stop_ap_cmd);
	}
						 ...
						 unsigned char wifi_nrf_vif_idx)
{
	...
	start_p2p_dev_cmd =
		wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*start_p2p_dev_cmd));
	...
	start_p2p_dev_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	start_p2p_dev_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	status = umac_cmd_cfg(fmac_dev_ctx, start_p2p_dev_cmd, sizeof(*start_p2p_dev_cmd));

out:
	if (start_p2p_dev_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, start_p2p_dev_cmd);
	}
						...
						unsigned char wifi_nrf_vif_idx)
{
	...
	stop_p2p_dev_cmd =
		wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*stop_p2p_dev_cmd));
	...
	stop_p2p_dev_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	stop_p2p_dev_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	status = umac_cmd_cfg(fmac_dev_ctx, stop_p2p_dev_cmd, sizeof(*stop_p2p_dev_cmd));
out:
	if (stop_p2p_dev_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, stop_p2p_dev_cmd);
	}
						 ...
						 struct remain_on_channel_info *roc_info)
{
	...
	roc_cmd->umac_hdr.ids.wdev_id = wifi_nrf_if_idx;
	roc_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &roc_cmd->info, roc_info,
			      sizeof(roc_cmd->info));
	...
	status = umac_cmd_cfg(fmac_dev_ctx, roc_cmd, sizeof(*roc_cmd));

out:
	if (roc_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, roc_cmd);
	}
						...
						unsigned long long cookie)
{
	...
	cancel_roc_cmd =
		wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*cancel_roc_cmd));
	...
	cancel_roc_cmd->umac_hdr.ids.wdev_id = wifi_nrf_if_idx;
	...
	status = umac_cmd_cfg(fmac_dev_ctx, cancel_roc_cmd, sizeof(*cancel_roc_cmd));
out:
	if (cancel_roc_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, cancel_roc_cmd);
	}
					   ...
					   struct img_umac_mgmt_tx_info *mgmt_tx_info)
{
	...
	mgmt_tx_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*mgmt_tx_cmd));
	...
	mgmt_tx_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	mgmt_tx_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &mgmt_tx_cmd->info, mgmt_tx_info,
			      sizeof(mgmt_tx_cmd->info));

	mgmt_tx_cmd->valid_fields |= (IMG_CMD_FRAME_FREQ_VALID | IMG_CMD_FRAME_DURATION_VALID |
	...
	status = umac_cmd_cfg(fmac_dev_ctx, mgmt_tx_cmd, sizeof(*mgmt_tx_cmd));

out:
	if (mgmt_tx_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, mgmt_tx_cmd);
	}
					   ...
					   struct img_umac_del_sta_info *del_sta_info)
{
	...
	del_sta_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*del_sta_cmd));
	...
	del_sta_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	del_sta_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &del_sta_cmd->info, del_sta_info,
			      sizeof(del_sta_cmd->info));

	if (!wifi_nrf_util_is_arr_zero(del_sta_info->mac_addr, sizeof(del_sta_info->mac_addr))) {
		del_sta_cmd->valid_fields |= IMG_CMD_DEL_STATION_MAC_ADDR_VALID;
	}

	if (del_sta_info->mgmt_subtype) {
		del_sta_cmd->valid_fields |= IMG_CMD_DEL_STATION_MGMT_SUBTYPE_VALID;
	}

	if (del_sta_info->reason_code) {
		del_sta_cmd->valid_fields |= IMG_CMD_DEL_STATION_REASON_CODE_VALID;
	}

	status = umac_cmd_cfg(fmac_dev_ctx, del_sta_cmd, sizeof(*del_sta_cmd));

out:
	if (del_sta_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, del_sta_cmd);
	}
					   ...
					   struct img_umac_add_sta_info *add_sta_info)
{
	...
	add_sta_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*add_sta_cmd));
	...
	add_sta_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	add_sta_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &add_sta_cmd->info, add_sta_info,
	...
	if (add_sta_info->sta_capability) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_STA_CAPABILITY_VALID;
	}

	add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_LISTEN_INTERVAL_VALID;

	if (add_sta_info->supp_rates.img_num_rates > 0) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_SUPP_RATES_VALID;
	}

	if (add_sta_info->ext_capability.ext_capability_len > 0) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_EXT_CAPABILITY_VALID;
	}

	if (add_sta_info->supported_channels.supported_channels_len > 0) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_SUPPORTED_CHANNELS_VALID;
	}

	if (add_sta_info->supported_oper_classes.supported_oper_classes_len > 0) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_SUPPORTED_OPER_CLASSES_VALID;
	}
	...
	if (!wifi_nrf_util_is_arr_zero(add_sta_info->ht_capability,
		...
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_HT_CAPABILITY_VALID;
	}

	if (!wifi_nrf_util_is_arr_zero(add_sta_info->vht_capability,
				       sizeof(add_sta_info->vht_capability))) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_VHT_CAPABILITY_VALID;
	}

	if (add_sta_info->opmode_notif) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_OPMODE_NOTIF_VALID;
	}

	if (add_sta_info->wme_uapsd_queues) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_STA_WME_UAPSD_QUEUES_VALID;
	}

	if (add_sta_info->wme_max_sp) {
		add_sta_cmd->valid_fields |= IMG_CMD_NEW_STATION_STA_WME_MAX_SP_VALID;
	}

	status = umac_cmd_cfg(fmac_dev_ctx, add_sta_cmd, sizeof(*add_sta_cmd));

out:
	if (add_sta_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, add_sta_cmd);
	}
					   ...
					   struct img_umac_chg_sta_info *chg_sta_info)
{
	...
	chg_sta_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*chg_sta_cmd));
	...
	chg_sta_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	chg_sta_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &chg_sta_cmd->info, chg_sta_info,
			      sizeof(chg_sta_cmd->info));

	if (chg_sta_info->aid) {
		chg_sta_cmd->valid_fields |= IMG_CMD_SET_STATION_AID_VALID;
	}

	if (chg_sta_info->supported_channels.supported_channels_len > 0) {
		chg_sta_cmd->valid_fields |= IMG_CMD_SET_STATION_SUPPORTED_CHANNELS_VALID;
	}

	if (chg_sta_info->supported_oper_classes.supported_oper_classes_len > 0) {
		chg_sta_cmd->valid_fields |= IMG_CMD_SET_STATION_SUPPORTED_OPER_CLASSES_VALID;
	}
	...
	if (chg_sta_info->opmode_notif) {
		chg_sta_cmd->valid_fields |= IMG_CMD_SET_STATION_OPMODE_NOTIF_VALID;
	}

	if (chg_sta_info->wme_max_sp) {
		chg_sta_cmd->valid_fields |= IMG_CMD_SET_STATION_STA_WME_MAX_SP_VALID;
	}

	status = umac_cmd_cfg(fmac_dev_ctx, chg_sta_cmd, sizeof(*chg_sta_cmd));

out:
	if (chg_sta_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, chg_sta_cmd);
	}
						  ...
						  struct img_umac_mgmt_frame_info *frame_info)
{
	...
	frame_reg_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	frame_reg_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &frame_reg_cmd->info, frame_info,
					    ...
					    unsigned char *addr)
{
	...
	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, addr, fmac_dev_ctx->base_mac_addr,
			      IMG_ETH_ALEN);

	if (((unsigned short)addr[5] + vif_idx) > 0xff) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: MAC Address rollover!!\n",
				      __func__);
	}
...
unsigned char wifi_nrf_fmac_add_vif(void *wifi_nrf_fmac_dev_ctx, void *os_vif_ctx,
				    struct img_umac_add_vif_info *vif_info)
{
	...
	case IMG_IFTYPE_P2P_GO:
		break;
	default:
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: VIF type not supported\n",
				      __func__);
		goto err;
	}

	if (wifi_nrf_fmac_vif_check_if_limit(fmac_dev_ctx, vif_info->iftype)) {
		goto err;
	}

	vif_ctx = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*vif_ctx));
	...
	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, vif_ctx->mac_addr, vif_info->mac_addr,
	...
	if (vif_idx != 0) {
		add_vif_cmd =
			wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*add_vif_cmd));
		...
		wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &add_vif_cmd->info, vif_info,
		...
		status = umac_cmd_cfg(fmac_dev_ctx, add_vif_cmd, sizeof(*add_vif_cmd));
...
err:
	if (vif_ctx) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, vif_ctx);
	}
...
out:
	if (add_vif_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, add_vif_cmd);
	}
					   ...
					   unsigned char wifi_nrf_vif_idx)
{
	...
	if (!vif_ctx) {
		wifi_nrf_osal_log_err(fmac_dev_ctx->fpriv->opriv, "%s: VIF ctx does not exist\n",
				      __func__);
		goto out;
	}
	 ...
	 * send commands for non-default interfaces
	 */
	if (wifi_nrf_vif_idx != 0) {
		del_vif_cmd =
			wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*del_vif_cmd));
		...
		del_vif_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
		del_vif_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

		status = umac_cmd_cfg(fmac_dev_ctx, del_vif_cmd, sizeof(*del_vif_cmd));
...
out:
	if (del_vif_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, del_vif_cmd);
	}
					   ...
					   struct img_umac_chg_vif_attr_info *vif_info)
{
	...
	chg_vif_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*chg_vif_cmd));
	...
	chg_vif_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	chg_vif_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &chg_vif_cmd->info, vif_info,
	...
	status = umac_cmd_cfg(fmac_dev_ctx, chg_vif_cmd, sizeof(*chg_vif_cmd));
out:
	if (chg_vif_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, chg_vif_cmd);
	}
						 ...
						 struct img_umac_chg_vif_state_info *vif_info)
{
	...
	chg_vif_state_cmd =
		wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*chg_vif_state_cmd));
	...
	chg_vif_state_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	chg_vif_state_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &chg_vif_state_cmd->info, vif_info,
	...
	status = umac_cmd_cfg(fmac_dev_ctx, chg_vif_state_cmd, sizeof(*chg_vif_state_cmd));
	...
	if (vif_ctx->if_type == IMG_IFTYPE_AP) {
		if (vif_info->state == 1) {
			...
			fmac_dev_ctx->tx_config.peers[MAX_PEERS].wifi_nrf_vif_idx =
				wifi_nrf_vif_idx;
...
out:
	if (chg_vif_state_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, chg_vif_state_cmd);
	}
...
enum wifi_nrf_status wifi_nrf_fmac_suspend(void *wifi_nrf_fmac_dev_ctx)
{
	...
	suspend_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*suspend_cmd));
	...
	status = umac_cmd_cfg(fmac_dev_ctx, suspend_cmd, sizeof(*suspend_cmd));
out:
	if (suspend_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, suspend_cmd);
	}
						...
						unsigned int wifi_nrf_vif_idx)
{
	...
	cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*cmd));
	...
	cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
...
out:
	if (cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, cmd);
	}
...
enum wifi_nrf_status wifi_nrf_fmac_get_station(void *wifi_nrf_fmac_dev_ctx,
					       unsigned int wifi_nrf_vif_idx, unsigned char *mac)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	...
	wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, cmd->info.mac_addr, mac, IMG_ETH_ALEN);
...
enum wifi_nrf_status wifi_nrf_fmac_resume(void *wifi_nrf_fmac_dev_ctx)
{
	...
	resume_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*resume_cmd));
	...
	status = umac_cmd_cfg(fmac_dev_ctx, resume_cmd, sizeof(*resume_cmd));
out:
	if (resume_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, resume_cmd);
	}
					       ...
					       struct img_umac_qos_map_info *qos_info)
{
	...
	set_qos_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*set_qos_cmd));
	...
	set_qos_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	set_qos_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;

	if (qos_info->qos_map_info_len) {
		wifi_nrf_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv, &set_qos_cmd->info.qos_map_info,
				      qos_info->qos_map_info, qos_info->qos_map_info_len);

		set_qos_cmd->info.qos_map_info_len = qos_info->qos_map_info_len;
	}

	status = umac_cmd_cfg(fmac_dev_ctx, set_qos_cmd, sizeof(*set_qos_cmd));
out:
	if (set_qos_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, set_qos_cmd);
	}
...
enum wifi_nrf_status wifi_nrf_fmac_set_power_save(void *wifi_nrf_fmac_dev_ctx,
						  unsigned char wifi_nrf_vif_idx, bool state)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	...
	set_ps_cmd = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*set_ps_cmd));
	...
	set_ps_cmd->umac_hdr.ids.wdev_id = wifi_nrf_vif_idx;
	set_ps_cmd->umac_hdr.ids.valid_fields |= IMG_INDEX_IDS_WDEV_ID_VALID;
	set_ps_cmd->info.ps_state = state;

	status = umac_cmd_cfg(fmac_dev_ctx, set_ps_cmd, sizeof(*set_ps_cmd));
out:
	if (set_ps_cmd) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, set_ps_cmd);
	}

	return status;
}

enum wifi_nrf_status wifi_nrf_fmac_set_wowlan(void *wifi_nrf_fmac_dev_ctx, unsigned int var)
{
	enum wifi_nrf_status status = WIFI_NRF_STATUS_FAIL;
	...
	set_wowlan = wifi_nrf_osal_mem_zalloc(fmac_dev_ctx->fpriv->opriv, sizeof(*set_wowlan));
	...
	status = umac_cmd_cfg(fmac_dev_ctx, set_wowlan, sizeof(*set_wowlan));
out:
	if (set_wowlan) {
		wifi_nrf_osal_mem_free(fmac_dev_ctx->fpriv->opriv, set_wowlan);
	}
