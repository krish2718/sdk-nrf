						  ...
						  void *hal_dev_ctx)
{
	...
	bal_dev_ctx = wifi_nrf_osal_mem_zalloc(bpriv->opriv, sizeof(*bal_dev_ctx));

	if (!bal_dev_ctx) {
		wifi_nrf_osal_log_err(bpriv->opriv, "%s: Unable to allocate bal_dev_ctx\n",
				      __func__);
		goto out;
	}
	...
	bal_dev_ctx->bus_dev_ctx = bpriv->ops->dev_add(bpriv->bus_priv, bal_dev_ctx);

	if (!bal_dev_ctx->bus_dev_ctx) {
		wifi_nrf_osal_log_err(bpriv->opriv, "%s: Bus dev_add failed\n", __func__);
		goto out;
	}
...
out:
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		if (bal_dev_ctx) {
			wifi_nrf_osal_mem_free(bpriv->opriv, bal_dev_ctx);
			bal_dev_ctx = NULL;
		}
	}
...
void wifi_nrf_bal_dev_rem(struct wifi_nrf_bal_dev_ctx *bal_dev_ctx)
{
	...
	wifi_nrf_osal_mem_free(bal_dev_ctx->bpriv->opriv, bal_dev_ctx);
}
...
enum wifi_nrf_status wifi_nrf_bal_dev_init(struct wifi_nrf_bal_dev_ctx *bal_dev_ctx)
{
	...
	if (status != WIFI_NRF_STATUS_SUCCESS) {
		wifi_nrf_osal_log_err(bal_dev_ctx->bpriv->opriv, "%s: dev_init failed\n", __func__);
		goto out;
	}
out:
	return status;
...
wifi_nrf_bal_init(struct wifi_nrf_osal_priv *opriv, struct wifi_nrf_bal_cfg_params *cfg_params,
		  enum wifi_nrf_status (*intr_callbk_fn)(void *hal_dev_ctx))
{
	struct wifi_nrf_bal_priv *bpriv = NULL;

	bpriv = wifi_nrf_osal_mem_zalloc(opriv, sizeof(*bpriv));

	if (!bpriv) {
		wifi_nrf_osal_log_err(opriv, "%s: Unable to allocate memory for bpriv\n", __func__);
		goto out;
	}
	...
	bpriv->bus_priv = bpriv->ops->init(opriv, cfg_params, &wifi_nrf_bal_isr);

	if (!bpriv->bus_priv) {
		wifi_nrf_osal_log_err(opriv, "%s: Failed\n", __func__);
		wifi_nrf_osal_mem_free(opriv, bpriv);
		bpriv = NULL;
	}
...
void wifi_nrf_bal_deinit(struct wifi_nrf_bal_priv *bpriv)
{
	...
	wifi_nrf_osal_mem_free(bpriv->opriv, bpriv);
}
...
unsigned int wifi_nrf_bal_read_word(void *ctx, unsigned long addr_offset)
{
	...
	val = bal_dev_ctx->bpriv->ops->read_word(bal_dev_ctx->bus_dev_ctx, addr_offset);
...
void wifi_nrf_bal_write_word(void *ctx, unsigned long addr_offset, unsigned int val)
{
	...
	bal_dev_ctx->bpriv->ops->write_word(bal_dev_ctx->bus_dev_ctx, addr_offset, val);
}
...
void wifi_nrf_bal_read_block(void *ctx, void *dest_addr, unsigned long src_addr_offset, size_t len)
{
	...
	bal_dev_ctx->bpriv->ops->read_block(bal_dev_ctx->bus_dev_ctx, dest_addr, src_addr_offset,
					    len);
}
			      ...
			      size_t len)
{
	...
	bal_dev_ctx->bpriv->ops->write_block(bal_dev_ctx->bus_dev_ctx, dest_addr_offset, src_addr,
					     len);
}
				   ...
				   enum wifi_nrf_osal_dma_dir dma_dir)
{
	...
	phy_addr =
		bal_dev_ctx->bpriv->ops->dma_map(bal_dev_ctx->bus_dev_ctx, virt_addr, len, dma_dir);
				     ...
				     enum wifi_nrf_osal_dma_dir dma_dir)
{
	...
	virt_addr = bal_dev_ctx->bpriv->ops->dma_unmap(bal_dev_ctx->bus_dev_ctx, phy_addr, len,
						       dma_dir);
