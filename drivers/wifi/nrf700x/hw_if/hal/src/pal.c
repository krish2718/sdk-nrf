...
enum wifi_nrf_status pal_rpu_addr_offset_get(struct wifi_nrf_osal_priv *opriv,
					     unsigned int rpu_addr, unsigned long *addr)
{
	...
	if (addr_base == RPU_ADDR_SBUS_START) {
	...
	} else if ((rpu_addr >= RPU_ADDR_GRAM_START) && (rpu_addr <= RPU_ADDR_GRAM_END)) {
		...
		wifi_nrf_osal_log_err(opriv, "%s: Invalid rpu_addr 0x%X\n", __func__, rpu_addr);
		goto out;
	}
...
char *pal_ops_get_fw_loc(struct wifi_nrf_osal_priv *opriv, enum wifi_nrf_fw_type fw_type,
			 enum wifi_nrf_fw_subtype fw_subtype)
{
	...
	case WIFI_NRF_FW_TYPE_LMAC_PATCH:
		if (fw_subtype == WIFI_NRF_FW_SUBTYPE_PRI) {
			...
			wifi_nrf_osal_log_err(opriv, "%s: Invalid LMAC FW sub-type = %d\n",
					      __func__, fw_subtype);
			goto out;
		}
		break;
	case WIFI_NRF_FW_TYPE_UMAC_PATCH:
		if (fw_subtype == WIFI_NRF_FW_SUBTYPE_PRI) {
			...
			wifi_nrf_osal_log_err(opriv, "%s: Invalid UMAC FW sub-type = %d\n",
					      __func__, fw_subtype);
			goto out;
		}
		break;
	default:
		wifi_nrf_osal_log_err(opriv, "%s: Invalid FW type = %d\n", __func__, fw_type);
		goto out;
	}
