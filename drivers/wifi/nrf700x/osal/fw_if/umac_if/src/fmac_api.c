...
#endif /* !CONFIG_NRF700X_RADIO_TEST */
{
	...
	fpriv->max_ac = fpriv->data_config.wmm ? WIFI_NRF_FMAC_AC_MAX : 1;
	fpriv->num_tx_tokens = MAX_TX_TOKENS;
	fpriv->num_tx_tokens_per_ac = (fpriv->num_tx_tokens / fpriv->max_ac);
	fpriv->num_tx_tokens_spare = (fpriv->num_tx_tokens % fpriv->max_ac);
	fpriv->spare_token_start = fpriv->num_tx_tokens_per_ac * fpriv->max_ac;
