...
#ifdef CONFIG_NRF_WIFI_RPU_RECOVERY_DEBUG
		LOG_ERR("%s: RPU recovery triggered in quiet period (last_rpu_recovery_time_ms=%ld), ignoring",
			__func__, rpu_ctx_zep->last_rpu_recovery_time_ms);
#else
		LOG_DBG("%s: RPU recovery triggered in quiet period (last_rpu_recovery_time_ms=%ld), ignoring",
			__func__, rpu_ctx_zep->last_rpu_recovery_time_ms);
#endif
		return;
	}
