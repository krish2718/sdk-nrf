...
static int cmd_wifi_status(const struct shell *shell, size_t argc, char *argv[])
{
	...
	shell_fprintf(shell, SHELL_NORMAL, "Status: successful\n");
	shell_fprintf(shell, SHELL_NORMAL, "==================\n");
	shell_fprintf(shell, SHELL_NORMAL, "State: %s\n", wifi_state_txt(status.state));

	if (status.state >= WIFI_ASSOCIATED) {
		uint8_t mac_string_buf[sizeof("xx:xx:xx:xx:xx:xx")];

		shell_fprintf(shell, SHELL_NORMAL, "Interface Mode: %s\n",
				wifi_mode_txt(status.iface_mode));
		shell_fprintf(shell, SHELL_NORMAL, "Link Mode: %s\n",
				wifi_link_mode_txt(status.link_mode));
		shell_fprintf(shell, SHELL_NORMAL, "SSID: %-32s\n", status.ssid);
		shell_fprintf(shell, SHELL_NORMAL, "BSSID: %s\n",
				status.bssid ? net_sprint_ll_addr_buf(status.bssid,
					WIFI_MAC_ADDR_LEN, mac_string_buf,
					sizeof(mac_string_buf)) :
				"");
		shell_fprintf(shell, SHELL_NORMAL, "Band: %s\n",
				wifi_band_txt(status.band));
		shell_fprintf(shell, SHELL_NORMAL, "Channel: %d\n", status.channel);
		shell_fprintf(shell, SHELL_NORMAL, "Security: %s\n",
				wifi_security_txt(status.security));
		shell_fprintf(shell, SHELL_NORMAL, "PMF: %s\n",
				wifi_mfp_txt(status.mfp));
		shell_fprintf(shell, SHELL_NORMAL, "RSSI: %d\n", status.rssi);
	}
...
static void print_wifi_stats(struct net_if *iface, struct net_stats_wifi *data,
			    const struct shell *shell)
{
	shell_fprintf(shell, SHELL_NORMAL, "Statistics for Wi-Fi interface %p [%d]\n", iface,
	       net_if_get_by_iface(iface));

	shell_fprintf(shell, SHELL_NORMAL, "Bytes received   : %u\n", data->bytes.received);
	shell_fprintf(shell, SHELL_NORMAL, "Bytes sent       : %u\n", data->bytes.sent);
	shell_fprintf(shell, SHELL_NORMAL, "Packets received : %u\n", data->pkts.rx);
	shell_fprintf(shell, SHELL_NORMAL, "Packets sent     : %u\n", data->pkts.tx);
	shell_fprintf(shell, SHELL_NORMAL, "Bcast received   : %u\n", data->broadcast.rx);
	shell_fprintf(shell, SHELL_NORMAL, "Bcast sent       : %u\n", data->broadcast.tx);
	shell_fprintf(shell, SHELL_NORMAL, "Mcast received   : %u\n", data->multicast.rx);
	shell_fprintf(shell, SHELL_NORMAL, "Mcast sent       : %u\n", data->multicast.tx);
	shell_fprintf(shell, SHELL_NORMAL, "Beacons received : %u\n", data->sta_mgmt.beacons_rx);
	shell_fprintf(shell, SHELL_NORMAL, "Beacons missed   : %u\n", data->sta_mgmt.beacons_miss);
}
#endif

static int cmd_wifi_stats(const struct shell *shell, size_t argc, char *argv[])
{
#if defined(CONFIG_NET_STATISTICS_WIFI) && \
					defined(CONFIG_NET_STATISTICS_USER_API)
	struct net_if *iface = net_if_get_default();
	...
	ret = net_mgmt(NET_REQUEST_STATS_GET_WIFI, iface,
				&stats, sizeof(stats));
	if (!ret) {
		print_wifi_stats(iface, &stats, shell);
	}
#else
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);

	shell_fprintf(shell, SHELL_INFO, "Set %s to enable %s support.\n",
...
SHELL_STATIC_SUBCMD_SET_CREATE(wifi_commands,
	SHELL_CMD(connect, NULL,
		  ...
		  "<PMF (optional): 0:Disable, 1:Optional, 2:Required",
		  cmd_wifi_connect),
