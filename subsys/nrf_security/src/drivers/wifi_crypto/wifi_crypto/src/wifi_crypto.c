...
static const uint32_t WIFI_CRYPTO_KEY_INDEX_INVALID = ~0;

static bool wifi_crypto_key_type_is_mic(wifi_crypto_key_type_t type)
{
	return (bool)(type & 1);
}

static uint32_t wifi_crypto_kmu_slot_id(wifi_crypto_key_type_t type, uint32_t db_id,
					uint32_t key_index)
{
	...
	if (wifi_crypto_key_type_is_mic(type)) {
...
static uint32_t wifi_crypto_get_key_start_addr(wifi_crypto_key_type_t type, uint32_t db_id,
					       uint32_t key_index)
{
	const uint32_t ram_base = 0x28400000;
	uint32_t db_base;
	uint32_t offset;
	bool mic = wifi_crypto_key_type_is_mic(type);

	if (type == VIF_ENC || type == VIF_MIC) {
		if (db_id >= 4) {
			return WIFI_CRYPTO_KEY_INDEX_INVALID;
		}
...
uint32_t wifi_crypto_get_key_size_in_bytes(wifi_crypto_key_type_t type)
{
	return wifi_crypto_key_type_is_mic(type) ? 16 : 32;
}

uint32_t wifi_crypto_get_key_size_in_bits(wifi_crypto_key_type_t type)
{
	return wifi_crypto_get_key_size_in_bytes(type) * 8;
}

static int wifi_crypto_kmu_provision_and_push(const uint32_t *key, wifi_crypto_key_type_t type,
					      uint32_t db_id, uint32_t key_index, uint32_t key_slot)
{
	...
	if (wifi_crypto_get_key_size_in_bytes(type) == 32) {
...
psa_key_attributes_t wifi_crypto_key_attributes_init(wifi_crypto_key_type_t type, uint32_t db_id,
						     uint32_t key_index)
{
	psa_key_attributes_t attr = PSA_KEY_ATTRIBUTES_INIT;
	wifi_crypto_set_key_id(&attr, db_id, key_index);
	...
	psa_key_lifetime_t lifetime = PSA_KEY_LIFETIME_FROM_PERSISTENCE_AND_LOCATION(
		persistence, PSA_KEY_LOCATION_WIFI_CRYPTO);
...
psa_status_t wifi_crypto_import_key(const psa_key_attributes_t *attr, const uint8_t *data,
				    ...
				    size_t *key_buffer_length, size_t *key_bits)
{
	...
	if (location == PSA_KEY_LOCATION_WIFI_CRYPTO) {
		...
		*key_bits = wifi_crypto_get_key_size_in_bits(type);
